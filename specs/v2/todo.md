...
The openhack server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@openhack-ai/llm` tool loop after replacing its
...
- plugins get global "openhack" instance like in that post i showed
- openhack instance has stuff like `openhack.session.prompt()` or
  `openhack.tool.register({...})`
...
- decide whether to preserve deprecated `@openhack-ai/llm` orchestration exports
