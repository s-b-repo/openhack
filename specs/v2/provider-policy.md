...
Both `action` and `resource` use openhack's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.openhack` files is intentionally deferred until `.openhack` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether openhack permits an operation through its managed execution paths.
