...
Move the canonical OpenHack terminal application from
`packages/openhack-cli/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/tui
name: @openhack-ai/tui
```

Target dependency graph:

```text
packages/openhack-cli ---\
                      > @openhack-ai/tui -> @openhack-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/openhack-cli`,
`packages/cli`, or `@openhack-ai/core`.

The SDK is the TUI's OpenHack boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/openhack-cli`.
- Do not replace private `packages/openhack-cli` imports with `@openhack-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@openhack-ai/tui`.

## Ownership Boundary

### `@openhack-ai/tui` Owns
...
- OpenHack domain data displayed by the TUI
...
```text
packages/openhack-cli/src/cli/cmd/tui
```

Its private dependency on `packages/openhack-cli` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/openhack-cli/src/*`.
...
Status: Completed. The private `@openhack-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@openhack-ai/tui`.
...
- The package has no dependency on `openhack`, `@openhack-ai/cli`, or
  `@openhack-ai/core`.
...
primitives now live in `@openhack-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@openhack-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads OpenHack globals or process state;
...
Replace process-global OpenHack state with resolved TUI inputs.
...
- Keep environment reads needed by legacy command or worker startup in
  `packages/openhack-cli` adapters.
...
`@openhack-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/openhack-cli` initially.
...
canonical in `@openhack-ai/tui`. Configured references resolve through the new
...
adapters rather than OpenHack domain implementations.

Make the SDK the only OpenHack domain boundary used by the TUI.
...
- Domain-facing TUI code imports OpenHack data and operations only from
  `@openhack-ai/sdk`.
...
`@openhack-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/openhack-cli`.
...
- The package has no imports from `packages/openhack-cli`, `packages/cli`, or
  `@openhack-ai/core`.
...
public `@openhack-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/openhack-cli`.
- Keep the legacy embedded worker and server startup in `packages/openhack-cli`.
- Change those adapters to load config, create transport inputs, and call the
  public `@openhack-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/openhack-cli`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@openhack-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@openhack-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for OpenHack domain state.
...
cd packages/openhack-cli && bun typecheck
...
rg '@openhack-ai/core|packages/openhack-cli|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/openhack-cli/src packages/openhack-cli/test
```
...
- Build the current-platform `packages/openhack-cli` binary.
