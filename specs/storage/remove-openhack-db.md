# Remove `packages/openhack-cli/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/openhack-cli/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the openhack legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/openhack-cli/src/storage/db.ts` are concentrated in 22 source files:

- `packages/openhack-cli/src/account/repo.ts`
- `packages/openhack-cli/src/cli/cmd/db.ts`
- `packages/openhack-cli/src/cli/cmd/import.ts`
- `packages/openhack-cli/src/cli/cmd/stats.ts`
- `packages/openhack-cli/src/control-plane/workspace.ts`
- `packages/openhack-cli/src/index.ts`
- `packages/openhack-cli/src/node.ts`
- `packages/openhack-cli/src/permission/index.ts`
- `packages/openhack-cli/src/project/project.ts`
- `packages/openhack-cli/src/server/projectors.ts`
- `packages/openhack-cli/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/openhack-cli/src/server/shared/fence.ts`
- `packages/openhack-cli/src/session/message-v2.ts`
- `packages/openhack-cli/src/session/projectors.ts`
- `packages/openhack-cli/src/session/prompt.ts`
- `packages/openhack-cli/src/session/session.ts`
- `packages/openhack-cli/src/session/todo.ts`
- `packages/openhack-cli/src/share/share-next.ts`
- `packages/openhack-cli/src/storage/db.ts`
- `packages/openhack-cli/src/sync/index.ts`
- `packages/openhack-cli/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy openhack database wrapper; `packages/openhack-cli/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the openhack wrapper.
...
Status: Completed. `SyncEvent` and the openhack projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy openhack database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy openhack database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy openhack database wrapper.
...
All migration groups are complete or superseded. `packages/openhack-cli/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No openhack data-migration group remains.

The previous openhack `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate openhack data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/openhack-cli`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/openhack-cli/src`
- `bun typecheck` from `packages/openhack-cli`
- Relevant package tests from `packages/openhack-cli`, not the repo root
