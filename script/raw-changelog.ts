...
const repo = process.env.GH_REPO ?? "anomalyco/openhack"
const bot = ["actions-user", "github-actions[bot]", "openhack", "openhack-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/openhack-cli packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/openhack-cli/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/openhack-cli/")) areas.add("core")
