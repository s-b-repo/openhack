...
import { createOpenhack } from "@openhack-ai/sdk"
...
`)
  ...
  const openhack = await createOpenhack({ port: 0 })
  ...
  try {
    ...
    const session = await openhack.client.session.create()
    const result = await openhack.client.session
      .prompt({
