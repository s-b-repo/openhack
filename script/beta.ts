...
const model = "openhack/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/openhack-cli")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with openhack...`)
  ...
  try {
    await $`openhack run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  openhack failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with openhack")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with openhack...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/openhack-cli`.",
