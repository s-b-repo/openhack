...
import { PromptInput } from "@openhack-ai/schema/prompt-input"
import { Session } from "@openhack-ai/schema/session"
import { Project } from "@openhack-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@openhack-ai/schema/schema"
...
import { SessionEvent } from "@openhack-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this OpenHack process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this OpenHack process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
