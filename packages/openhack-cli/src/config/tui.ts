...
import { isRecord } from "@openhack-ai/tui/util/record"
...
import { TuiKeybind } from "@openhack-ai/tui/config/keybind"
...
import { TuiConfig } from "@openhack-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@openhack/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old openhack.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.openhack`
  // folders between cwd and home, and OPENHACK_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.OPENHACK_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit OPENHACK_TUI_CONFIG override, if set.
  if (Flag.OPENHACK_TUI_CONFIG) {
    const configFile = Flag.OPENHACK_TUI_CONFIG
  ...
  // 4. `.openhack` directories (and OPENHACK_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".openhack") || dir === Flag.OPENHACK_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".openhack") && dir !== Flag.OPENHACK_CONFIG_DIR) continue
