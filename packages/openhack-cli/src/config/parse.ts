...
import type { DeepMutable } from "@openhack-ai/core/schema"
import { InvalidError, JsonError } from "@openhack-ai/core/v1/config/error"
...
// OpenHack engine config namespaces. The unified `.openhack/openhack.jsonc` holds
...
function topLevelExtraKeys(schema: EffectSchema.Top, data: unknown) {
  ...
  return Object.keys(data).filter((key) => !known.has(key) && !OPENHACK_ENGINE_KEYS.has(key))
}
