...
const TUI_SCHEMA_URL = "https://cybersec.org.za/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from openhack.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const openhack = await openhackFiles(input)
  for (const file of openhack) {
...
async function openhackFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "openhack"),
    ...(await Filesystem.findUp(["openhack.json", "openhack.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "openhack"))
  }
  if (Flag.OPENHACK_CONFIG) files.push(Flag.OPENHACK_CONFIG)
