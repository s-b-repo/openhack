...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@openhack-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@openhack-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@openhack-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@openhack-ai/tui/context/theme"
import { Global } from "@openhack-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@openhack-ai/core/util/flock"
import { Flag } from "@openhack-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@openhack-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@openhack-ai/core/v1/config/plugin"
import { createCommandShim } from "@openhack-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@openhack-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".openhack"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".openhack", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".openhack", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.OPENHACK_PURE ? [] : pluginOrigins
    if (Flag.OPENHACK_PURE && pluginOrigins.length) {
    }
