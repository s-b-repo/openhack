...
export const DEPRECATED_PLUGIN_PACKAGES = ["openhack-openai-codex-auth", "openhack-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, openhackVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(openhackVersion) || semver.major(openhackVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.openhack
  if (typeof range !== "string") return
  if (!semver.satisfies(openhackVersion, range)) {
    throw new Error(`Plugin requires openhack ${range} but running ${openhackVersion}`)
  }
}
