...
import type { AssistantMessage, Message, OpenhackClient, SessionMessageResponse } from "@openhack-ai/sdk/v2"
...
export const AuthMethodID = "openhack-login"
...
export class Service extends Context.Service<Service, Interface>()("@openhack/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `openhack auth login` in the terminal",
      name: "Login with openhack",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: OpenhackClient) {
...
function makeUsageService(sdk: OpenhackClient) {
...
async function loadDirectorySnapshot(sdk: OpenhackClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, openhack provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const openhackProvider = providers[ProviderV2.ID.make("openhack")]
  const openhackModel = openhackProvider ? Provider.sort(Object.values(openhackProvider.models))[0] : undefined
  if (openhackProvider && openhackModel) return { providerID: openhackProvider.id, modelID: openhackModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "OpenHack service failure", service })
}
