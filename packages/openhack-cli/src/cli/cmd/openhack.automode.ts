...
export function listMacros(): string[] {
  const dirs = [path.join(process.cwd(), ".openhack", "command")]
  if (process.env.HOME) dirs.push(path.join(process.env.HOME, "openhack", ".openhack", "command"))
...
export async function runOrchestrationLoop(target: string, opts: LoopOptions): Promise<Automode.AutomodeSession> {
  ...
  //    the .openhack/command/<X>.md file is the single source of truth for the
