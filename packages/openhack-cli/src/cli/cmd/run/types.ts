// Shared type vocabulary for the direct interactive mode (`openhack --mini`).
//
...
import type { OpenhackClient, PermissionRequest, QuestionRequest, ToolPart } from "@openhack-ai/sdk/v2"
import type { TuiConfig } from "@openhack-ai/tui/config"
...
type PromptModel = Parameters<OpenhackClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<OpenhackClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<OpenhackClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<OpenhackClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<OpenhackClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<OpenhackClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<OpenhackClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<OpenhackClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<OpenhackClient["question"]["reject"]>[0]
