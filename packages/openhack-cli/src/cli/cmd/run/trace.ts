...
//
// Enable with OPENHACK_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/openhack/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.OPENHACK_DIRECT_TRACE) {
    state = false
    return undefined
  }
