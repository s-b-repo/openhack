import type { PermissionV1 } from "@openhack-ai/core/v1/permission"
import { FSUtil } from "@openhack-ai/core/fs-util"
// CLI entry point for `openhack run` and `openhack --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`openhack --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`openhack --mini --attach`): connects to a running
//      openhack server and runs interactive mode against it.
//
...
import { createOpenhackClient, type OpenhackClient, type ToolPart } from "@openhack-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<OpenhackClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run openhack with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running openhack server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to OPENHACK_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENHACK_SERVER_USERNAME or 'openhack')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: OpenhackClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: OpenhackClient, sessionID: string) {
      ...
      async function current(sdk: OpenhackClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: OpenhackClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: OpenhackClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: OpenhackClient) {
        ...
        async function loop(client: OpenhackClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createOpenhackClient({
        baseUrl: "http://openhack.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
