...
import type { LLMClientShape } from "@openhack-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("openhack"))
    return { type: "unsupported", reason: "provider is not openai, openhack, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @openhack-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains openhack-owned. The native runtime only adapts
      // the @openhack-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
