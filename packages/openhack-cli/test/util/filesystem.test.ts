...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "openhack.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "openhack.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openhack.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openhack.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["openhack.json", "openhack.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "openhack.json"),
        path.join(project, "openhack.jsonc"),
      ])
    })
  })
