...
const original = {
  ...
  envPassword: process.env.OPENHACK_SERVER_PASSWORD,
  envUsername: process.env.OPENHACK_SERVER_USERNAME,
}
const auth = { username: "openhack", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.OPENHACK_SERVER_PASSWORD
  else process.env.OPENHACK_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.OPENHACK_SERVER_USERNAME
  else process.env.OPENHACK_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.OPENHACK_SERVER_PASSWORD = auth.password
  Flag.OPENHACK_SERVER_USERNAME = auth.username
  process.env.OPENHACK_SERVER_PASSWORD = auth.password
  process.env.OPENHACK_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.OPENHACK_SERVER_USERNAME = auth.username
  delete process.env.OPENHACK_SERVER_PASSWORD
  process.env.OPENHACK_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-openhack-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-openhack-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-openhack-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("default in-process handler does not emit Effect HTTP response logs", async () => {
    ...
    try {
      // Hit a real in-process API route (not the UI catch-all, which proxies to
      // the cybersec.org.za upstream and needs network); this test only cares that
      // a successful in-process response emits no "Sent HTTP response" log.
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.OPENHACK_DISABLE_DEFAULT_PLUGINS
    process.env.OPENHACK_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.OPENHACK_DISABLE_DEFAULT_PLUGINS
      else process.env.OPENHACK_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-openhack-ticket": "1" },
      })
