  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("OPENHACK_SERVER_PASSWORD", original.envPassword)
        restoreEnv("OPENHACK_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
...
function authConfigLayer(input?: { password?: string; username?: string }) {
  return ServerAuth.Config.configLayer({
    ...
    username: input?.username ?? "openhack",
  })
}
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            OPENHACK_SERVER_PASSWORD: input?.password,
            OPENHACK_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  it.live("serves the web UI through the HTTP API app", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({
        ...
        client: httpClient(
          new Response("<html>openhack</html>", { headers: { "content-type": "text/html" } }),
          (request) => {
            proxiedUrl = request.url
          },
        ),
      ...
      expect(yield* responseText(response)).toBe("<html>openhack</html>")
      expect(proxiedUrl).toBe("https://cybersec.org.za/")
    }),
  )
  ...
  it.live("strips upstream transfer encoding headers from proxied assets", () =>
    Effect.gen(function* () {
      ...
      expect(response.status).toBe(200)
      expect(proxiedUrl).toBe("https://cybersec.org.za/assets/app.js")
            ...
            Layer.succeed(
              ...
              HttpClient.make((request) =>
                Effect.succeed(
                  HttpClientResponse.fromWeb(
                    request,
                    new Response("<html>openhack</html>", {
                      headers: {
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        username: "openhack",
        disableEmbeddedWebUi: true,
        client: httpClient(new Response("<html>openhack</html>", { headers: { "content-type": "text/html" } })),
      }).request(`/?auth_token=${btoa("openhack:secret")}`)
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("openhack:secret")}` },
      })
  ...
  it.live("accepts basic auth passwords containing colons for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("openhack:sec:ret")}` },
      })
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "openhack" }).request("/", {
        method: "OPTIONS",
        headers: {
