...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-openhack-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-openhack-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://cybersec.org.za/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-openhack-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-openhack-workspace")
