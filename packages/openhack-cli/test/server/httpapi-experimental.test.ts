...
import { AccountV2 } from "@openhack-ai/core/account"
import { AccountTable } from "@openhack-ai/core/account/sql"
...
function withCreatedWorktree(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      expect(info).toMatchObject({ name, branch: "openhack/api-test" })
      ...
      Effect.gen(function* () {
        ...
        const [consoleState, consoleOrgs, toolList, toolIDs, worktrees, resources] = yield* Effect.all(
          [
            ...
            request(`${ExperimentalPaths.tool}?provider=openhack&model=gpt-5`, directory),
