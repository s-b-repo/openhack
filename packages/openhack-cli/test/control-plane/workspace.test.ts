...
import { EventSequenceTable } from "@openhack-ai/core/event/sql"
...
import { WorkspaceV2 } from "@openhack-ai/core/workspace"
import { WorkspaceTable } from "@openhack-ai/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@openhack-ai/core/ripgrep"
...
const originalEnv = {
  OPENHACK_AUTH_CONTENT: process.env.OPENHACK_AUTH_CONTENT,
  OPENHACK_EXPERIMENTAL_WORKSPACES: process.env.OPENHACK_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.OPENHACK_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@openhack.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.OPENHACK_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=openhack-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.OPENHACK_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.OPENHACK_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.OPENHACK_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=openhack-test")
