// Subprocess integration tests for `openhack run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `openhack.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `OPENHACK_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("openhack run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, openhack }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openhack.run("say hi")
        openhack.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, openhack }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openhack.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, openhack }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* openhack.run("think", { extraArgs: ["--thinking"] })
        openhack.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* openhack.run("think again")
        openhack.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, openhack }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openhack.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, openhack }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openhack.run("say hi", { format: "json" })
        openhack.expectExit(result, 0)

        const events = openhack.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ openhack }) =>
      Effect.gen(function* () {
        const result = yield* openhack.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, openhack }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openhack.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, openhack }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openhack.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, openhack }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* openhack.run("request permission", { permission: { bash: "ask" } })
        openhack.expectExit(denied, 0)
        ...
        const allowed = yield* openhack.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        openhack.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* openhack.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        openhack.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, openhack }) =>
      Effect.gen(function* () {
        ...
        const server = yield* openhack.serve()

        const result = yield* openhack.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, openhack }) =>
      Effect.gen(function* () {
        const result = yield* openhack.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, openhack }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* openhack.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
