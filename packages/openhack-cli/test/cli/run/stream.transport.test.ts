...
import { OpenhackClient, type GlobalEvent } from "@openhack-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<OpenhackClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<OpenhackClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<OpenhackClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<OpenhackClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<OpenhackClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://openhack.test"),
...
function sdk(
  input: {
    ...
    subscribe?: OpenhackClient["event"]["subscribe"]
    globalEvent?: OpenhackClient["global"]["event"]
    promptAsync?: OpenhackClient["session"]["promptAsync"]
    status?: OpenhackClient["session"]["status"]
    messages?: OpenhackClient["session"]["messages"]
    children?: OpenhackClient["session"]["children"]
    permissions?: OpenhackClient["permission"]["list"]
    questions?: OpenhackClient["question"]["list"]
  } = {},
) {
  const client = new OpenhackClient()

  const subscribe: OpenhackClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: OpenhackClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: OpenhackClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: OpenhackClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: OpenhackClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: OpenhackClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: OpenhackClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: OpenhackClient["question"]["list"] = input.questions ?? (() => ok([]))
