...
describe("openhack read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // OPENHACK_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ openhack }) =>
      Effect.gen(function* () {
        const r = yield* openhack.spawn(["mcp", "list"])
        openhack.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openhack }) =>
      Effect.gen(function* () {
        const r = yield* openhack.spawn(["providers", "list"])
        openhack.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ openhack }) =>
      Effect.gen(function* () {
        const r = yield* openhack.spawn(["models"])
        openhack.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ openhack }) =>
      Effect.gen(function* () {
        const r = yield* openhack.spawn(["agent", "list"])
        openhack.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh OPENHACK_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ openhack }) =>
      Effect.gen(function* () {
        const r = yield* openhack.spawn(["session", "list"])
        openhack.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openhack }) =>
      Effect.gen(function* () {
        const r = yield* openhack.spawn(["stats"])
        openhack.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under OPENHACK_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ openhack }) =>
      Effect.gen(function* () {
        const r = yield* openhack.spawn(["db", "path"])
        openhack.expectExit(r, 0, "db path")
