...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ openhack }) =>
    Effect.gen(function* () {
      const result = yield* openhack.spawn(["--replay-limit", "10"])

      openhack.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ openhack }) =>
    Effect.gen(function* () {
      const result = yield* openhack.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ openhack }) =>
    Effect.gen(function* () {
      const result = yield* openhack.spawn(["--mini", "--port", "4096"])
