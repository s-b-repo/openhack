...
describe("openhack acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ openhack }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ openhack }))
        ...
        expect(initialized.agentInfo?.name).toBe("OpenHack")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openhack }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ openhack })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("openhack-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "openhack-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.OPENHACK_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openhack }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("openhack-login")
