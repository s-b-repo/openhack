...
import type { Event, OpenhackClient } from "@openhack-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<OpenhackClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "openhack-acp-permission-"))
