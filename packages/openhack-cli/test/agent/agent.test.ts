...
it.instance("plan agent denies edits except .openhack/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".openhack/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".openhack", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OPENHACK_TEST_HOME
      process.env.OPENHACK_TEST_HOME = test.directory
