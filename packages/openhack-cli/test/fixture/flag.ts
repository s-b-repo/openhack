...
/**
 * Scoped override for `Flag.OPENHACK_WORKSPACE_ID`. Saves the previous value
...
export function withFixedWorkspaceID(id: WorkspaceV2.ID): Effect.Effect<void, never, Scope.Scope> {
  return Effect.gen(function* () {
    const previous = Flag.OPENHACK_WORKSPACE_ID
    Flag.OPENHACK_WORKSPACE_ID = id
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        Flag.OPENHACK_WORKSPACE_ID = previous
      }),
    )
  })
}
