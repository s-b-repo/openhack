...
import { LLMEvent, ToolFailure } from "@openhack-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@openhack-ai/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("openhack"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("openhack") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("openhack") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, openhack, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored openhack auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("openhack") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("openhack"),
