...
import { SessionV1 } from "@openhack-ai/core/v1/session"
import { ModelsDev } from "@openhack-ai/core/models-dev"
import { HttpRecorder } from "@openhack-ai/http-recorder"
import { HttpRecorderInternal } from "@openhack-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@openhack-ai/llm"
import { RequestExecutor } from "@openhack-ai/llm/route"
...
const zenURL = (connection: string) => `https://cybersec.org.za/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.OPENHACK_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["openhack", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OPENHACK_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("OPENHACK_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["openhack", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "openhack-proxy",
    name: "OpenHack proxy",
    providerID: ProviderV2.ID.openhack,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["openhack", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.OPENHACK_RECORD_CONSOLE_TOKEN && process.env.OPENHACK_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openhack,
        name: "OpenHack Zen",
        env: ["OPENHACK_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.OPENHACK_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.OPENHACK_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.OPENHACK_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["openhack", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OPENHACK_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("OPENHACK_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("OPENHACK_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set OPENHACK_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the openhack LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "openhack.json"),
      JSON.stringify({ $schema: "https://cybersec.org.za/config.json", ...scenario.config(model) }),
    ),
  )
