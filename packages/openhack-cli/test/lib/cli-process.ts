// Subprocess test harness for the openhack CLI. Spawns the real binary against
...
//
// Configuration flows through openhack's built-in test affordances:
//   - OPENHACK_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENHACK_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENHACK_DISABLE_PROJECT_CONFIG : skip walking up for openhack.json
//   - OPENHACK_PURE                : skip external plugin discovery + install
//   - OPENHACK_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `openhack.run` is fully wired. The shape supports adding more
// builders (`openhack.serve(opts)`, `openhack.acp(opts)`, `openhack.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpenhackCli.
...
import { AppProcess } from "@openhack-ai/core/process"
...
const openhackRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(openhackRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    OPENHACK_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    OPENHACK_CONFIG_CONTENT: configJson,
    OPENHACK_DISABLE_PROJECT_CONFIG: "1",
    OPENHACK_PURE: "1",
    OPENHACK_DISABLE_AUTOUPDATE: "1",
    OPENHACK_DISABLE_AUTOCOMPACT: "1",
    OPENHACK_DISABLE_MODELS_FETCH: "1",
    OPENHACK_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `openhack run`. New flags should
...
// `openhack serve` is a long-lived process — it never exits on its own.
...
// `openhack acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpenhackCli = {
  ...
  // Spawn `openhack serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `openhack acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("openhack.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          OPENHACK_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("openhack.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("openhack.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "openhack server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `openhack serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("openhack.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const openhack: OpenhackCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, openhack })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `openhack.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "openhack") {
...
// every call site. Use this for any test that needs the openhack CLI fixture.
//
...
// resources (e.g. `openhack.serve`) without an extra `Effect.scoped` wrapper —
