...
Practical reference for a future `packages/server` split after the openhack
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/openhack-cli`.
...
Until enough shared service code lives outside `packages/openhack-cli`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/openhack-cli`

It should not import `packages/openhack-cli` services while `packages/openhack-cli`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on openhack-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/openhack-cli` implementation details.
