...
Practical reference for the current tool-migration state in `packages/openhack-cli`.
...
Tool tests should use the existing Effect helpers in `packages/openhack-cli/test/lib/effect.ts`:
