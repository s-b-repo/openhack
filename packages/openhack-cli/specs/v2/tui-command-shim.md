...
Delete `packages/openhack-cli/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/openhack-cli/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/openhack-cli/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/openhack-cli`
- TUI plugin loader tests in `packages/openhack-cli` if runtime plugin API wiring changed
