...
- Author package entrypoint is `@openhack-ai/plugin/tui`.
...
```json
{
  "$schema": "https://cybersec.org.za/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/openhack-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@openhack-ai/plugin/tui`.
- `@openhack-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@openhack-ai/plugin/tui"
...
- The value is a semver range checked against the running OpenHack version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.openhack` is absent, no check is performed (backward compatible).
...
- `openhack plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `openhack plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.openhack` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.openhack`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.openhack`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.openhack` dir or `OPENHACK_CONFIG_DIR`, OpenHack installs `@openhack-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@openhack-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus OpenHack's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
OpenHack registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, OpenHack pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@openhack-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"openhack"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `openhack.default`.
...
- Local plugins persist installed themes under the local `.openhack/themes` area near the plugin config source.
...
- `--pure` / `OPENHACK_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.openhack/plugins/tui-smoke.tsx`
- Local vim plugin: `.openhack/plugins/tui-vim.tsx`
- Local smoke config: `.openhack/tui.json`
- Local smoke theme: `.openhack/plugins/smoke-theme.json`
