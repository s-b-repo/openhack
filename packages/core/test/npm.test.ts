...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@openhack/acme")).toBe("@openhack/acme")
    expect(Npm.sanitize("@openhack/acme@1.0.0")).toBe("@openhack/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/openhack/acme.git"
    const expected = win ? "acme@git+https_//github.com/openhack/acme.git" : spec
