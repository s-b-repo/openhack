...
import { ConfigProvider } from "@openhack-ai/core/config/provider"
...
import { ConfigMigrateV1 } from "@openhack-ai/core/v1/config/migrate"
...
import { Policy } from "@openhack-ai/core/policy"
import { Project } from "@openhack-ai/core/project"
import { AbsolutePath } from "@openhack-ai/core/schema"
...
describe("Config", () => {
  ...
  it.live("loads openhack JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "openhack.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "openhack.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "openhack.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "openhack.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "openhack.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "openhack.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "openhack.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .openhack configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".openhack"), { recursive: true })
            await fs.mkdir(path.join(directory, ".openhack"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "openhack.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "openhack.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "openhack.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "openhack.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "openhack.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".openhack", "openhack.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".openhack", "openhack.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".openhack")),
              AbsolutePath.make(path.join(directory, ".openhack")),
            ])
