...
const openhackClient = process.env.OPENHACK_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (openhackClient === undefined) delete process.env.OPENHACK_CLIENT
  else process.env.OPENHACK_CLIENT = openhackClient
})
...
describe("resource", () => {
  ...
  test("drops OTEL resource attributes when any entry is invalid", () => {
    ...
    expect(resource().attributes["openhack.client"]).toBeDefined()
  })

  test("keeps built-in attributes when env values conflict", () => {
    process.env.OPENHACK_CLIENT = "cli"
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "openhack.client=web,service.instance.id=override,service.namespace=anomalyco"
    ...
    expect(resource().attributes["openhack.run"]).toMatch(/^[0-9a-f]{8}$/)
  })
})

test("file logger appends concurrent runs with a run on every line", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openhack-log-test-"))
  await using _ = {
    async [Symbol.asyncDispose]() {
      await fs.rm(dir, { recursive: true, force: true })
    },
  }
  const file = path.join(dir, "openhack.log")
