...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openhack-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["OPENHACK_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { OPENHACK_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("OPENHACK_TEST_SHELL=ok")
      }),
    )
