...
import { Watcher } from "@openhack-ai/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    OPENHACK_EXPERIMENTAL_FILEWATCHER: "true",
    OPENHACK_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@openhack.test`.cwd(tmp.path).quiet()
