...
import { define } from "@openhack-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@openhack-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "openhack.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "openhack.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
