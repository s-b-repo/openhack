...
import { GoogleVertexPlugin } from "@openhack-ai/core/plugin/provider/google-vertex"
  ...
  it.effect("ignores OpenAI-compatible providers that are not Google Vertex", () =>
    Effect.gen(function* () {
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.openhack, (provider) => {
          provider.api = {
            type: "aisdk",
            package: "@ai-sdk/openai-compatible",
            url: "https://cybersec.org.za/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.openhack))
