...
import { ConfigDcr } from "./config/dcr"
...
export class Info extends Schema.Class<Info>("Config.Info")({
  ...
  dcr: ConfigDcr.Info.pipe(Schema.optional).annotate({
    description:
      "Dynamic Context Runtime: budgeted working-set assembly over unbounded session history (dcr-serve sidecar)",
  }),
...
export class Service extends Context.Service<Service, Interface>()("@openhack/v2/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["openhack.json", "openhack.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".openhack", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".openhack")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".openhack").toReversed()
    ...
    // global config, project files, then `.openhack` files.
