...
import type { IntegrationOAuthMethodRegistration } from "@openhack-ai/plugin/v2/effect/integration"
import { define } from "@openhack-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@openhack-ai/sdk/v2/types"
...
const defaultServer = "https://cybersec.org.za"
const clientID = "openhack-cli"
...
export const OpenhackPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "openhack",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpenhackPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("openhack")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load OpenHack provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("openhack", (integration) => {
        integration.name = "OpenHack"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "openhack", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("openhack")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("openhack")
      ...
      const item = catalog.provider.get(ProviderV2.ID.openhack)
      if (!item) return
      const hasKey = Boolean(process.env.OPENHACK_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("openhack")),
