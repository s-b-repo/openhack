...
import { ToolFailure } from "@openhack-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@openhack/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.OPENHACK_WEBSEARCH_PROVIDER === "exa" || process.env.OPENHACK_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.OPENHACK_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("OPENHACK_EXPERIMENTAL") || truthy("OPENHACK_ENABLE_EXA") || truthy("OPENHACK_EXPERIMENTAL_EXA"),
    enableParallel: truthy("OPENHACK_ENABLE_PARALLEL") || truthy("OPENHACK_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `openhack/${InstallationVersion}`,
