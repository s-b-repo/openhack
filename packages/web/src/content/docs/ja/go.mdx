...
OpenHack Goは低価格のサブスクリプションで、**初月は5ドル**、その後は**月額10ドル**で、人気のオープンなコーディングモデルに安定してアクセスできます。

GoはOpenHackの他のプロバイダーと同様に機能します。OpenHack GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、OpenHackを使用するために必須ではありません。
...
:::tip
OpenHackでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
OpenHack Goを使用すると、これらのモデルに**初月は5ドル**、その後は**月額10ドル**でアクセスできます。

---

## 仕組み

OpenHack Goは、OpenHackの他のプロバイダーと同様に機能します。

1. **<a href={console}>OpenHack Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`OpenHack Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
OpenHack Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
OpenHack設定の[model id](/docs/config/#models)は、`openhack-go/<model-id>`という形式を使用します。たとえば、Kimi K2.7 Codeの場合は、設定で`openhack-go/kimi-k2.7-code`を使用します。
...
4. OpenHackで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
