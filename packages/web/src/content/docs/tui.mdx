---
title: TUI
description: Using the OpenHack terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenHack provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenHack starts the TUI for the current directory.
...
When using the OpenHack TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenHack. Allows you to select from available providers and add their API keys.
...
Exit OpenHack. _Aliases_: `/quit`, `/q`
...
This is separate from `openhack.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long OpenHack waits after the leader key. Defaults to `2000`.
...
Use `OPENHACK_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `openhack.default`.
