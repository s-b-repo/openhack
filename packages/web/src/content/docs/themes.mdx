...
With OpenHack you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenHack uses our own `openhack` theme.
...
OpenHack comes with several built-in themes.
...
- Want OpenHack to match their terminal's appearance
...
OpenHack supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/openhack/themes/*.json` or `$XDG_CONFIG_HOME/openhack/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.openhack/themes/*.json`
4. **Current working directory** - Defined in `./.openhack/themes/*.json`
