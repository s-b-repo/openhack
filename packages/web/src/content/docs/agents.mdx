...
There are two types of agents in OpenHack; primary agents and subagents.
...
OpenHack comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
OpenHack comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

OpenHack comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into OpenHack's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `openhack.json` config file:
...
If no temperature is specified, OpenHack uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global OpenHack config and the project specific config.
...
The model ID in your OpenHack config uses the format `provider/model-id`. For example, if you're using [OpenHack Zen](/docs/zen), you would use `openhack/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `openhack models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/openhack).
:::
