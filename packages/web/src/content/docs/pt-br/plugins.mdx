---
title: Plugins
description: Escreva seus próprios plugins para estender o openhack.
---

Plugins permitem que você estenda o openhack conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do openhack.
...
- `.openhack/plugins/` - Plugins em nível de projeto
- `~/.config/openhack/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/openhack/node_modules/`.
...
1. Configuração global (`~/.config/openhack/openhack.json`)
2. Configuração do projeto (`openhack.json`)
3. Diretório de plugins global (`~/.config/openhack/plugins/`)
4. Diretório de plugins do projeto (`.openhack/plugins/`)
...
O openhack executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do openhack para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o openhack.
...
:::note
Se você estiver usando o aplicativo desktop openhack, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o openhack de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao openhack:
...
O helper `tool` cria uma ferramenta personalizada que o openhack pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o openhack junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://cybersec.org.za/docs/sdk) para detalhes.
