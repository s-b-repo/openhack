...
As ferramentas permitem que o LLM execute ações em sua base de código. O openhack vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no openhack.
...
:::note
Esta ferramenta está disponível apenas quando `OPENHACK_EXPERIMENTAL_LSP_TOOL=true` (ou `OPENHACK_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor openhack ou quando a variável de ambiente `OPENHACK_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o openhack:
