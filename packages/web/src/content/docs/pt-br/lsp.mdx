...
description: O openhack integra-se aos seus servidores LSP.
---

O openhack pode se integrar a servidores Language Server Protocol (LSP) para usar diagnósticos como feedback para o agente.

---

## Integrados

O openhack vem com vários servidores LSP integrados para linguagens populares:
...
:::note
Você pode desabilitar os downloads automáticos do servidor LSP definindo a variável de ambiente `OPENHACK_DISABLE_LSP_DOWNLOAD` como `true`.
:::
...
Quando o LSP está habilitado e o openhack abre um arquivo, ele:
...
Você pode habilitar e personalizar os servidores LSP através da seção `lsp` na sua configuração do openhack.
