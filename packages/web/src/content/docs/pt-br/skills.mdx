...
As habilidades do agente permitem que o openhack descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O openhack pesquisa nesses locais:

- Configuração do projeto: `.openhack/skills/<name>/SKILL.md`
- Configuração global: `~/.config/openhack/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o openhack sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.openhack/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/openhack/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.openhack/skills/git-release/SKILL.md` assim:
...
O openhack lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `openhack.json`:
...
**Para agentes embutidos** (em `openhack.json`):
