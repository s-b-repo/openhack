...
Existem dois tipos de agentes no openhack; agentes primários e subagentes.
...
openhack vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
OpenHack vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

OpenHack vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do OpenHack, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `openhack.json`:
...
- Global: `~/.config/openhack/agents/`
- Por projeto: `.openhack/agents/`

```markdown title="~/.config/openhack/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o openhack usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do openhack quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do openhack usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenHack Zen](/docs/zen), você usaria `openhack/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="openhack.json" {3-6,9-12}
{
  "$schema": "https://cybersec.org.za/config.json",
  "tools": {
    "write": true,
...
```json title="openhack.json" {3-5,8-10}
{
  "$schema": "https://cybersec.org.za/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `openhack models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/openhack).
:::
...
```markdown title="~/.config/openhack/agents/docs-writer.md"
---
...
```markdown title="~/.config/openhack/agents/security-auditor.md"
---
