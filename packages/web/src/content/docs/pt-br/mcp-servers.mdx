...
Você pode adicionar ferramentas externas ao openhack usando o _Model Context Protocol_, ou MCP. O openhack suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do openhack](https://cybersec.org.za/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/openhack`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O openhack lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o openhack irá:
...
Se o servidor requer autenticação, o openhack solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `openhack mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o openhack armazenará os tokens de forma segura em `~/.local/share/openhack/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no openhack, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do openhack como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o openhack à sua conta do Sentry.
