...
Com o openhack, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o openhack usa nosso próprio tema `openhack`.
...
O openhack vem com vários temas embutidos.
...
- Querem que o openhack corresponda à aparência do seu terminal
...
O openhack suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/openhack/themes/*.json` ou `$XDG_CONFIG_HOME/openhack/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.openhack/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.openhack/themes/*.json`
