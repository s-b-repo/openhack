...
description: Crie ferramentas que o LLM pode chamar no openhack.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do openhack, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.openhack/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/openhack/tools/`.
