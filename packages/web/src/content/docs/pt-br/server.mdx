---
title: Servidor
description: Interaja com o servidor openhack via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `openhack serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente openhack pode usar.
...
```bash
openhack serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `openhack.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
openhack serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENHACK_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `openhack`, ou defina `OPENHACK_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `openhack serve` quanto ao `openhack web`.

```bash
OPENHACK_SERVER_PASSWORD=your-password openhack serve
```
...
Quando você executa `openhack`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor openhack para interagir com o openhack programaticamente.
:::

Essa arquitetura permite que o openhack suporte múltiplos clientes e permite que você interaja com o openhack programaticamente.

Você pode executar `openhack serve` para iniciar um servidor autônomo. Se você tiver o TUI do openhack em execução, `openhack serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do openhack [IDE](/docs/ide).
...
O servidor openhack expõe as seguintes APIs.
