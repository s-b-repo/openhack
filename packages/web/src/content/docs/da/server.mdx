---
title: Server
description: Interager med openhack-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openhack serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en openhack-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `openhack.local` |
...
Indstil `OPENHACK_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `openhack`, eller indstil `OPENHACK_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `openhack serve` og `openhack web`.
...
Når du kører `openhack` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug openhack-serveren til at interagere med openhack programmatisk.
:::

Denne arkitektur lader openhack understøtte flere klienter og lader dig interagere med openhack programmatisk.

Du kan køre `openhack serve` for at starte en selvstændig server. Hvis du har
openhack TUI kørende, vil `openhack serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af OpenHack [IDE](/docs/ide) plugins.
...
OpenHack-serveren viser følgende API'er.
