...
Agentferdigheter lar OpenHack oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
OpenHack søker etter disse stedene:

- Prosjektkonfigurasjon: `.openhack/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/openhack/skills/<name>/SKILL.md`
...
For prosjektlokale stier går OpenHack opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.openhack/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/openhack/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.openhack/skills/git-release/SKILL.md` slik:
...
OpenHack viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `openhack.json`:
...
**For innebygde agenter** (i `openhack.json`):
