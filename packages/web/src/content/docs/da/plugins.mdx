---
title: Plugins
description: Skriv dine egne plugins for at udvide OpenHack.
---

Plugins giver dig mulighed for at udvide OpenHack ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenHacks standardadfærd.
...
- `.openhack/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/openhack/node_modules/`.
...
1. Global konfiguration (`~/.config/openhack/openhack.json`)
2. Projektkonfiguration (`openhack.json`)
3. Global plugin-mappe (`~/.config/openhack/plugins/`)
4. Projekt plugin bibliotek (`.openhack/plugins/`)
...
OpenHack kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En openhack SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide openhack.
...
:::note
Hvis du bruger OpenHack desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå openhack i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til openhack:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som openhack kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for openhack sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://cybersec.org.za/docs/sdk) for detaljer.
