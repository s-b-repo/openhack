...
Med OpenHack kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OpenHack vårt eget `openhack`-tema.
...
OpenHack kommer med flere innebygde temaer.
...
- Vil at OpenHack skal matche terminalens utseende
...
OpenHack støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/openhack/themes/*.json` eller `$XDG_CONFIG_HOME/openhack/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.openhack/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.openhack/themes/*.json`
