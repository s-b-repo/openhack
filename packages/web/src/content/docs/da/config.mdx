---
title: Konfiguration
description: Ved at bruge OpenHack JSON konfig.
---

Du kan konfigurere OpenHack ved hjælp af en JSON-konfigurationsfil.

---

## Format

OpenHack understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/openhack`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/openhack/openhack.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENHACK_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`openhack.json` i projekt) - projektspecifikke indstillinger
5. **`.openhack` mapper** - agent, kommandoer, plugins
6. **Inline config** (`OPENHACK_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.openhack` og `~/.config/openhack` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/openhack`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale OpenHack-konfiguration i `~/.config/openhack/openhack.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `openhack.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når OpenHack starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `OPENHACK_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `OPENHACK_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.openhack` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.openhack` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`cybersec.org.za/config.json`**](https://cybersec.org.za/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `openhack serve` og `openhack web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din OpenHack-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `openhack.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din OpenHack-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger OpenHack at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din OpenHack-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/openhack/agents/` eller `.openhack/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil OpenHack falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`openhack run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/openhack/commands/` eller `.openhack/commands/`. [Learn more here](/docs/commands).
...
OpenHack vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard openhack **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide OpenHack med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.openhack/plugins/` eller `~/.config/openhack/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse OpenHack til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
