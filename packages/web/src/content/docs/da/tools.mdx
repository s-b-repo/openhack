...
Her er alle de innebygde verktøyene som er tilgængelige i OpenHack.
...
:::note
Dette verktøyet er bare tilgængelig når `OPENHACK_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENHACK_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenHack-leverandøren eller når miljøvariabelen `OPENHACK_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
