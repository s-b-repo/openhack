---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OpenHack.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHack bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OpenHack-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/openhack/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i OpenHack-konfigurationen.
...
OpenHack Zen er en liste over modeller leveret af OpenHack-teamet, som er blevet testet og verificeret til at fungere godt med OpenHack. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenHack Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg openhack og gå til [cybersec.org.za/auth](https://cybersec.org.za/auth).
...
Det fungerer som alle andre udbydere i OpenHack og er helt valgfrit at bruge.

---

## OpenHack Go

OpenHack Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af OpenHack-teamet, som er testet og verificeret til at fungere godt med OpenHack.

1. Kør kommandoen `/connect` i TUI, vælg `OpenHack Go`, og gå til [cybersec.org.za/auth](https://cybersec.org.za/zen).
...
Sådan bruger du Amazon Bedrock med OpenHack:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører openhack:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `openhack.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OpenHack understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere openhack til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at openhack skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem openhack-konfigurationen.
...
gemme tokenet i openhack auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
OpenHack bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OpenHack
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`openhack.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med OpenHack:
...
Sådan bruger du Google Vertex AI med OpenHack:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører openhack.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem openhack, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [openhack-helicone-session](https://github.com/H2Shami/openhack-helicone-session) plugin for automatisk at logge hver OpenHack-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OpenHack-samtale opført som en separat session.
...
Du kan konfigurere openhack til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere openhack til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere openhack til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenHack. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/openhack) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OpenHack:
...
3. Kopier API-nøglen til brug i OpenHack.
...
6. **Vigtigt**: Før du bruger skymodeller i OpenHack, skal du hente modelinformationen lokalt:
...
OpenHack Zen er en liste over testede og verificerede modeller leveret af OpenHack-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OpenHack Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OpenHack Zen**.

   ```txt
   /connect
   ```

3. Indtast OpenHack API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem openhack-konfigurationen.
...
5. Du kan også tilpasse dem gennem openhack-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenHack:
...
Du kan også tilpasse modeller gennem openhack-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med openhack. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `openhack.json`-filen i projektkataloget:
...
`limit`-felterne lader OpenHack forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `openhack auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek openhack-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i openhack-konfigurationen.
