...
For at fejlfinde problemer med OpenHack, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openhack\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `openhack --log-level DEBUG`.

---

## Lagring

openhack gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/openhack/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openhack`
...
OpenHack Desktop kører en lokal OpenHack-server (`openhack-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `OpenHack`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/openhack/openhack.jsonc` (eller `~/.config/openhack/openhack.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/openhack/openhack.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openhack\openhack.jsonc`
...
OpenHack kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/openhack/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openhack\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så OpenHack kan genopbygge den.

1. Afslut OpenHack Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/openhack`
- **Linux**: slet `~/.cache/openhack` (eller kør `rm -rf ~/.cache/openhack`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\openhack`

3. Start OpenHack Desktop på ny.

---

### Løs problemer med serverforbindelse

OpenHack Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `openhack.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `OPENHACK_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `OPENHACK_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver OpenHack Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i OpenHack.

---

### Meddelelser vises ikke

OpenHack Desktop viser kun systemvarsler når:

- varsler er aktiveret for OpenHack i dine OS-indstillinger, og
...
1. Afslut OpenHack Desktop.
2. Find og slet disse filer (de findes i OpenHack Desktop-appens datamappe):

- `openhack.settings.dat` (skrivebordsstandardserver URL)
- `openhack.global.dat` og `openhack.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med OpenHack:
...
For at finde ud af hvilke modeller du har adgang til, kør `openhack models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\openhack`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. openhack installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\openhack`

2. Start openhack på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge openhack til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
openhack vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
