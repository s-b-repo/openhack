---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OpenHack.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHack bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OpenHack-konfigurasjonen.
...
i `~/.local/share/openhack/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OpenHack
konfig.
...
OpenHack Zen er en liste over modeller levert av OpenHack-teamet som har vært
testet og verifisert for å fungere godt med OpenHack. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenHack Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `OpenHack Zen` og gå til [cybersec.org.za/auth](https://cybersec.org.za/zen).
...
Det fungerer som alle andre leverandører i OpenHack og er helt valgfritt å bruke.

---

## OpenHack Go

OpenHack Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av OpenHack-teamet som har vært
testet og verifisert for å fungere godt med OpenHack.

1. Kjør kommandoen `/connect` i TUI, velg `OpenHack Go`, og gå til [cybersec.org.za/auth](https://cybersec.org.za/zen).
...
Slik bruker du Amazon Bedrock med OpenHack:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OpenHack:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `openhack.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i openhack støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere openhack til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OpenHack skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OpenHack-konfigurasjonen.
...
for å lagre token i OpenHack auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OpenHack bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OpenHack
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`openhack.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med OpenHack:
...
Slik bruker du Google Vertex AI med OpenHack:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører OpenHack.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OpenHack, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [openhack-helicone-session](https://github.com/H2Shami/openhack-helicone-session) plugin for automatisk å logge hver OpenHack-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OpenHack-samtale oppført som en separat økt.
...
Du kan konfigurere OpenHack for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OpenHack for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OpenHack for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OpenHack. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/openhack) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OpenHack:
...
3. Kopier API-nøkkelen for bruk i OpenHack.
...
6. **Viktig**: Før du bruker skymodeller i OpenHack, må du hente modellinformasjonen lokalt:
...
OpenHack Zen er en liste over testede og verifiserte modeller levert av OpenHack-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OpenHack Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OpenHack Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OpenHack API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OpenHack-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OpenHack-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenHack:
...
Du kan også tilpasse modeller gjennom OpenHack-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OpenHack. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `openhack.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OpenHack forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `openhack auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OpenHack-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OpenHack-konfigurasjonen.
