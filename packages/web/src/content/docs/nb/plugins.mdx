...
description: Skriv dine egne programtillegg for å utvide OpenHack.
---

Plugins lar deg utvide OpenHack ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenHack.
...
- `.openhack/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/openhack/node_modules/`.
...
1. Global konfigurasjon (`~/.config/openhack/openhack.json`)
2. Prosjektkonfigurasjon (`openhack.json`)
3. Global plugin-katalog (`~/.config/openhack/plugins/`)
4. Prosjektpluginkatalog (`.openhack/plugins/`)
...
OpenHack kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenHack SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenHack.
...
:::note
Hvis du bruker OpenHack-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenHack fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenHack:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenHack kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenHack sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://cybersec.org.za/docs/sdk) for detaljer.
