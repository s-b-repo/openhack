...
Du kan legge til eksterne verktøy i OpenHack ved å bruke _Model Context Protocol_ eller MCP. OpenHack støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenHack Config](https://cybersec.org.za/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="openhack.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/openhack`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
OpenHack håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenHack:
...
Hvis serveren krever autentisering, vil OpenHack be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `openhack mcp auth <server-name>`.
...
```bash
openhack mcp auth my-oauth-server
```
...
```bash
openhack mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenHack lagre tokens sikkert i `~/.local/share/openhack/mcp-auth.json`.
...
openhack mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i OpenHack, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenHack-konfigurasjonen som alle andre verktøy.
...
```json title="openhack.json" {11, 14-18}
{
  "$schema": "https://cybersec.org.za/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
openhack mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenHack til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenHack-samtale oppført som en separat økt.
