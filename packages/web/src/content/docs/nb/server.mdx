---
title: Tjener
description: Samhandle med OpenHack-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openhack serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenHack-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `openhack.local` |
...
Sett `OPENHACK_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `openhack` som standard, eller sett `OPENHACK_SERVER_USERNAME` for å overstyre det. Dette gjelder både `openhack serve` og `openhack web`.
...
Når du kjører `openhack` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OpenHack-serveren til å samhandle med OpenHack programmatisk.
:::

Denne arkitekturen lar OpenHack støtte flere klienter og lar deg samhandle med OpenHack programmatisk.

Du kan kjøre `openhack serve` for å starte en frittstående server. Hvis du har
OpenHack TUI kjører, vil `openhack serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av OpenHack [IDE](/docs/ide) plugins.
...
openhack-serveren viser følgende APIer.
