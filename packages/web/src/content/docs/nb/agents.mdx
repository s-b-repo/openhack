...
Det er to typer agenter i OpenHack; primære agenter og underagenter.
...
OpenHack kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
OpenHack kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

OpenHack kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `openhack.json`:
...
Hvis ingen temperatur er spesifisert, bruker OpenHack modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenHack-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i OpenHack-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenHack Zen](/docs/zen), vil du bruke `openhack/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `openhack models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/openhack).
:::
