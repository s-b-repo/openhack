---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć openhack.
---

Wtyczki umożliwiające wyłączenie openhack poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania openhack.
...
- `.openhack/plugins/` - Wtyczki na poziomie projektu
- `~/.config/openhack/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/openhack/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/openhack/openhack.json`)
2. Konfiguracja projektu (`openhack.json`)
3. Globalny katalog wtyczek (`~/.config/openhack/plugins/`)
4. Katalog wtyczek projektu (`.openhack/plugins/`)
...
openhack uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z openhack do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do openhack.
...
:::note
Jeśli korzystasz z aplikacji komputerowej openhack, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj openhack czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do openhack:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod openhack. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://cybersec.org.za/docs/sdk).
