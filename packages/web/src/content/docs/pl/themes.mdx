...
Dzięki openhack możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie openhack używa naszego własnego motywu `openhack`.
...
openhack ma kilka wbudowanych motywów.
...
- Chcą, aby OpenHack pasował do wyglądu ich terminala
...
openhack obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/openhack/themes/*.json` lub `$XDG_CONFIG_HOME/openhack/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.openhack/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.openhack/themes/*.json`
