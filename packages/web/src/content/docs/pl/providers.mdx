---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w openhack.
---

import config from "../../../../config.mjs"
export const console = config.console

openhack używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji openhack.
...
w `~/.local/share/openhack/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim openhack
config.
...
OpenHack Zen to lista modeli dostarczonych przez zespół openhack, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z openhack. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenHack Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenHack Zen` i przejdź do [cybersec.org.za/auth](https://cybersec.org.za/zen).
...
Działa jak każdy inny dostawca w openhack i jest całkowicie opcjonalny w użyciu.

---

## OpenHack Go

OpenHack Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół openhack, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z openhack.

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenHack Go` i przejdź do [cybersec.org.za/auth](https://cybersec.org.za/zen).
...
Aby używać Amazon Bedrock z openhack:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania openhack:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `openhack.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w openhack nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować openhack do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod openhack działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji openhack.
...
openhack używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować openhack
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`openhack.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem openhack:
...
Aby używać Google Vertex AI z openhack:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania openhack.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą openhack, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [openhack-helicone-session](https://github.com/H2Shami/openhack-helicone-session), aby automatycznie rejestrować każdą konwersację openhack jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja openhack będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować openhack tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować openhack tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować openhack tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem openhack. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/openhack).
:::
...
Aby korzystać z Ollama Cloud z openhack:
...
3. Skopiuj klucz API do użycia w openhack.
...
6. **Ważne**: Przed użyciem modeli chmurowych w openhack musisz pobrać informacje o modelu lokalnie:
...
OpenHack Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół openhack. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OpenHack Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OpenHack Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API openhack.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji openhack.
...
5. Możesz je także dostosować za pomocą konfiguracji openhack. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z openhack:
...
Możesz także dostosować modele za pomocą konfiguracji openhack. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z openhack. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `openhack.json` w katalogu projektu:
...
Pola `limit` pozwalają openhack zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `openhack auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację openhack i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji openhack.
