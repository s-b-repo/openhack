...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. OpenHack posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w OpenHack.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OPENHACK_EXPERIMENTAL_LSP_TOOL=true` (lub `OPENHACK_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenHack lub gdy zmienna środowiskowa `OPENHACK_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu OpenHack:
