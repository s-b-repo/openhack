...
Możesz dodać dodatkowe informacje do openhack za pomocą _Model Context Protocol_, w skrócie MCP. openhack obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [openhack Config](https://cybersec.org.za/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/openhack`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
openhack automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, openhack:
...
Jeśli serwer wymaga uwierzytelnienia, openhack poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `openhack mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji openhack bezpieczne przechowa tokeny w `~/.local/share/openhack/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w openhack, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie openhack, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć openhack z kontem Sentry.
