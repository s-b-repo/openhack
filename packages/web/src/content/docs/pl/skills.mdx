...
Umiejętności agenta pozwalają openhack odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
openhack przeszukuje te lokalizacje:

- Project config: `.openhack/skills/<name>/SKILL.md`
- Global config: `~/.config/openhack/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu openhack przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.openhack/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/openhack/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.openhack/skills/git-release/SKILL.md` w ten sposób:
...
openhack wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `openhack.json`:
...
**Dla agentów wbudowanych** (w `openhack.json`):
