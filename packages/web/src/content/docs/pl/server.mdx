---
title: Serwer
description: Interakcja z serwerem openhack za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `openhack serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient openhack.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `openhack.local` |
...
Ustaw `OPENHACK_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `openhack` lub ustaw `OPENHACK_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `openhack serve`, jak i `openhack web`.
...
Kiedy uruchomisz `openhack`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera openhack do programowej interakcji z openhack.
:::

Ta architektura umożliwia obsługę wielu klientów przez openhack i programową interakcję z kodem otwartym.

Możesz uruchomić `openhack serve`, aby uruchomić samodzielny serwer. Jeśli masz
openhack TUI działa, `openhack serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki openhack [IDE](/docs/ide).
...
Serwer openhack udostępnia następujące interfejsy API.
