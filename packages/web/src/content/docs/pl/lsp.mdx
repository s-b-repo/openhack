---
title: Serwery LSP
description: OpenHack integruje się z serwerami LSP.
---

OpenHack może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

OpenHack posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `OPENHACK_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i OpenHack otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji OpenHack.
