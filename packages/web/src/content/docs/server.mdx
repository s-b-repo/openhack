---
title: Server
description: Interact with openhack server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `openhack serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an openhack client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `openhack.local` |
...
Set `OPENHACK_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `openhack`, or set `OPENHACK_SERVER_USERNAME` to override it. This applies to both `openhack serve` and `openhack web`.
...
When you run `openhack` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the openhack server to interact with openhack programmatically.
:::

This architecture lets openhack support multiple clients and allows you to interact with openhack programmatically.

You can run `openhack serve` to start a standalone server. If you have the
openhack TUI running, `openhack serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenHack [IDE](/docs/ide) plugins.
...
The openhack server exposes the following APIs.
