---
title: Policies
description: Control which configured resources OpenHack may use.
---

Policies control whether OpenHack may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `openhack.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether OpenHack may use a resource such as an LLM provider.
...
OpenHack currently supports one policy action:
