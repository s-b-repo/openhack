---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in OpenHack.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHack verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer OpenHack-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem OpenHack anpassen
config.
...
OpenHack Zen ist eine Liste von Modellen, die vom OpenHack-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit OpenHack funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit OpenHack Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie OpenHack aus und gehen Sie zu [cybersec.org.za/auth](https://cybersec.org.za/auth).
...
Es funktioniert wie jeder andere Anbieter in OpenHack und ist völlig optional.

---

## OpenHack Go

OpenHack Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom OpenHack-Team getestet und verifiziert wurden, dass sie gut mit OpenHack funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `OpenHack Go` und gehen Sie zu [cybersec.org.za/auth](https://cybersec.org.za/zen).
...
So verwenden Sie Amazon Bedrock mit OpenHack:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie OpenHack ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `openhack.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in OpenHack wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können openhack so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit OpenHack ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre OpenHack-Konfiguration hinzufügen.
...
um das Token im OpenHack-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
OpenHack verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von OpenHack
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`openhack.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit OpenHack:
...
So verwenden Sie Google Vertex AI mit OpenHack:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie OpenHack ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über OpenHack konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [openhack-helicone-session](https://github.com/H2Shami/openhack-helicone-session)-Plugin, um jede OpenHack-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede OpenHack-Konversation als separate Sitzung aufgeführt.
...
Sie können OpenHack für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können OpenHack über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können OpenHack über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für OpenHack konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/openhack).
:::
...
So verwenden Sie Ollama Cloud mit OpenHack:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in OpenHack.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in OpenHack verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
OpenHack Zen ist eine Liste getesteter und verifizierter Modelle, die vom OpenHack-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>OpenHack Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **OpenHack Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren OpenHack API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre OpenHack-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre OpenHack-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit OpenHack:
...
Sie können Modelle auch über Ihre OpenHack-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit OpenHack verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `openhack.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann OpenHack erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `openhack auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die OpenHack-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer OpenHack-Konfiguration übereinstimmt.
