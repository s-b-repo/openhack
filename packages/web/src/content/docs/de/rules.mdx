---
title: Regeln
description: Lege eigene Anweisungen fuer openhack fest.
---

Du kannst openhack ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in openhack den Befehl `/init` aus.
...
So kann openhack sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht openhack sie zu erweitern.
...
openhack kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/openhack/AGENTS.md` speichern.
Diese gelten in allen openhack-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OpenHack dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/openhack/AGENTS.md` existiert)
...
Beim Start sucht openhack in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/openhack/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `openhack.json` oder in `~/.config/openhack/openhack.json` angeben.
...
openhack parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit openhack.json

Empfohlen ist das `instructions`-Feld in der `openhack.json`:
...
Du kannst openhack explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `openhack.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
