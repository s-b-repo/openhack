...
Agent Skills erlauben OpenHack, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OpenHack sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OpenHack vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.openhack/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/openhack/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.openhack/skills/git-release/SKILL.md` zum Beispiel so:
...
OpenHack listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `openhack.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `openhack.json`):
