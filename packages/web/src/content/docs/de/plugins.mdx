---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OpenHack zu erweitern.
---

Mit Plugins können Sie OpenHack erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OpenHack zu ändern.
...
- `.openhack/plugins/` – Plugins auf Projektebene
- `~/.config/openhack/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/openhack/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/openhack/openhack.json`)
2. Projektkonfiguration (`openhack.json`)
3. Globales Plugin-Verzeichnis (`~/.config/openhack/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.openhack/plugins/`)
...
OpenHack führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OpenHack-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OpenHack erweitern können.
...
:::note
Wenn Sie die OpenHack-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OpenHack `.env`-Dateien liest:
...
Plugins können OpenHack auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OpenHack aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OpenHack zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://cybersec.org.za/docs/sdk).
