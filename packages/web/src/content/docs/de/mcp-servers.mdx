...
Sie können externe Tools zu OpenHack hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. OpenHack unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [OpenHack Config](https://cybersec.org.za/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/openhack`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
OpenHack übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird OpenHack Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von OpenHack beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `openhack mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert OpenHack die Token sicher in `~/.local/share/openhack/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in OpenHack verfügbar. Sie können sie also wie jedes andere Tool über die OpenHack-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und OpenHack mit Ihrem Sentry-Konto zu verbinden.
