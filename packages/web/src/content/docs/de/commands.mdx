...
Erstellen Sie `.openhack/commands/test.md`:

```md title=".openhack/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die OpenHack-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem OpenHack [config](/docs/config):

```json title="openhack.jsonc" {4-12}
{
...
- Global: `~/.config/openhack/commands/`
- Pro Projekt: `.openhack/commands/`

```markdown title="~/.config/openhack/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".openhack/commands/component.md"
---
...
```md title=".openhack/commands/create-file.md"
---
...
```md title=".openhack/commands/analyze-coverage.md"
---
...
```md title=".openhack/commands/review-changes.md"
---
...
```md title=".openhack/commands/review-component.md"
---
...
OpenHack enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
