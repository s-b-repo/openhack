...
Mit OpenHack kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet OpenHack das `openhack`-Theme.
...
OpenHack liefert mehrere eingebaute Themes mit.
...
- OpenHack optisch ans Terminal anpassen wollen
...
OpenHack unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/openhack/themes/*.json` oder `$XDG_CONFIG_HOME/openhack/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.openhack/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.openhack/themes/*.json`
