---
title: LSP Server
description: OpenHack lässt sich in Ihren LSP-Server integrieren.
---

OpenHack kann sich in Language Server Protocol (LSP)-Server integrieren, um Diagnosen als Feedback für den Agenten zu verwenden.

---

## Integriert

OpenHack verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `OPENHACK_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn LSP aktiviert ist und OpenHack eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer OpenHack-Konfiguration aktivieren und anpassen.
