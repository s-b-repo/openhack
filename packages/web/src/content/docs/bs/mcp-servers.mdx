...
Možete dodati vanjske alate u OpenHack koristeći _Model Context Protocol_, ili MCP. OpenHack podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenHack Config](https://cybersec.org.za/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/openhack`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OpenHack automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenHack će:
...
Ako server zahtijeva autentifikaciju, OpenHack će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `openhack mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenHack sigurno čuva tokene u `~/.local/share/openhack/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenHack, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenHack konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenHack sa vašim Sentry nalogom.
