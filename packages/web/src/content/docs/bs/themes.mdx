...
U OpenHack mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OpenHack koristi nasu `openhack` temu.
...
OpenHack dolazi sa vise ugradenih tema.
...
- Zele da OpenHack odgovara izgledu njihovog terminala
...
OpenHack podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/openhack/themes/*.json` ili `$XDG_CONFIG_HOME/openhack/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.openhack/themes/*.json`
4. **Trenutni radni direktorij** - `./.openhack/themes/*.json`
