...
Agent skills omogucavaju OpenHack da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
OpenHack pretrazuje ove lokacije:

- Konfiguracija projekta: `.openhack/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/openhack/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, OpenHack ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.openhack/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/openhack/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.openhack/skills/git-release/SKILL.md` ovako:
...
OpenHack navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `openhack.json`:
...
**Za ugradene agente** (u `openhack.json`):
