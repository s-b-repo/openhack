---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OpenHack.
---

Dodaci vam omogućavaju da proširite OpenHack spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OpenHack.
...
- `.openhack/plugins/` - Dodaci na nivou projekta
- `~/.config/openhack/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/openhack/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/openhack/openhack.json`)
2. Konfiguracija projekta (`openhack.json`)
3. Globalni direktorij dodataka (`~/.config/openhack/plugins/`)
4. Direktorij dodataka projekta (`.openhack/plugins/`)
...
OpenHack pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Openhack SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OpenHack.
...
Spriječite openhack da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u openhack:
...
Pomoćnik `tool` kreira prilagođeni alat koji openhack može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za openhack zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://cybersec.org.za/docs/sdk) za detalje.
