...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. OpenHack dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u OpenHack.
...
:::note
Ovaj alat je dostupan samo kada je `OPENHACK_EXPERIMENTAL_LSP_TOOL=true` (ili `OPENHACK_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenHack provajdera ili kada je varijabla `OPENHACK_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju OpenHack:
