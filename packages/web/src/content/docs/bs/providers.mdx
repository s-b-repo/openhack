---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenHack.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHack koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenHack konfiguraciji.
...
u `~/.local/share/openhack/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenHack
config.
...
OpenHack Zen je lista modela koje je obezbedio OpenHack tim koji su bili
testirano i potvrđeno da dobro radi sa OpenHack. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenHack Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite openhack i idite na [cybersec.org.za/auth](https://cybersec.org.za/auth).
...
Radi kao i svaki drugi provajder u OpenHack i potpuno je opcionalan za korištenje.

---

## OpenHack Go

OpenHack Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža OpenHack tim i koji su testirani i verificirani da dobro rade s OpenHack-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenHack Go` i idite na [cybersec.org.za/auth](https://cybersec.org.za/zen).
...
Da biste koristili Amazon Bedrock s OpenHack:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete openhack:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `openhack.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenHack.
:::
...
Možete konfigurirati openhack za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi openhack ispravno radio.
   :::
...
Također možete dodati modele kroz svoju openhack konfiguraciju.
...
da pohrani token u openhack auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OpenHack koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenHack
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`openhack.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `openhack.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s openhack:
...
Za korištenje Google Vertex AI s OpenHack:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće openhack.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem openhacka, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [openhack-helicone-session](https://github.com/H2Shami/openhack-helicone-session) da automatski prijavite svaki OpenHack razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenHack razgovor naveden kao zasebna sesija.
...
Možete konfigurirati openhack za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati openhack za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati openhack za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenHack. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/openhack) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenHack:
...
3. Kopirajte API ključ za korištenje u OpenHack.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenHack, morate lokalno povući informacije o modelu:
...
OpenHack Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenHack tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenHack Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenHack Zen**.

```txt
   /connect
```

3. Unesite svoj OpenHack API ključ.
...
Također možete dodati dodatne modele putem vaše openhack konfiguracije.
...
5. Također ih možete prilagoditi putem vaše openhack konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Openhackom:
...
Također možete prilagoditi modele kroz svoju openhack konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s openhack-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `openhack.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OpenHack da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `openhack auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenHack konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj openhack konfiguraciji.
