...
Da biste otklonili probleme s OpenHack, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openhack\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `openhack --log-level DEBUG`.

---

## Pohrana

OpenHack pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/openhack/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openhack`
...
OpenHack Desktop pokreće lokalni OpenHack server (`openhack-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OpenHack` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/openhack/openhack.jsonc` (ili `~/.config/openhack/openhack.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/openhack/openhack.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openhack\openhack.jsonc`
...
OpenHack također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/openhack/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openhack\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OpenHack mogao ponovo izgraditi.

1. Potpuno zatvorite OpenHack Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/openhack`
- **Linux**: obrišite `~/.cache/openhack` (ili pokrenite `rm -rf ~/.cache/openhack`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\openhack`

3. Ponovo pokrenite OpenHack Desktop.

---

### Rješavanje problema sa vezom na serveru

OpenHack Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `openhack.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OPENHACK_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OPENHACK_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, OpenHack Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OpenHack karakteristikama.

---

### Obavještenja se ne prikazuju

OpenHack Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OpenHack u postavkama vašeg OS-a, i
...
1. Zatvorite OpenHack Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OpenHack Desktop aplikacije):

- `openhack.settings.dat` (URL zadanog servera za desktop)
- `openhack.global.dat` i `openhack.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `openhack models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\openhack`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OpenHack dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\openhack`

2. Ponovo pokrenite OpenHack da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OpenHack da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OpenHack će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
