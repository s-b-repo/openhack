...
Postoje dvije vrste agenata u OpenHack: primarni agenti i podagenti.
...
OpenHack dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
OpenHack dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

OpenHack dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u OpenHack-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `openhack.json`:
...
- Globalno: `~/.config/openhack/agents/`
- Po projektu: `.openhack/agents/`
...
Ako temperatura nije navedena, OpenHack koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu OpenHack konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj OpenHack konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenHack Zen](/docs/zen), koristili biste `openhack/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `openhack models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/openhack).
:::
