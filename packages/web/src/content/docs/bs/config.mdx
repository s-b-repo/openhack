---
title: Konfiguracija
description: Korištenje OpenHack JSON konfiguracije.
---

Možete konfigurirati OpenHack koristeći JSON konfiguracijski fajl.

---

## Format

OpenHack podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/openhack`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/openhack/openhack.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENHACK_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`openhack.json` u projektu) - postavke specifične za projekat
5. **`.openhack` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OPENHACK_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.openhack` i `~/.config/openhack` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/openhack` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu OpenHack konfiguraciju u `~/.config/openhack/openhack.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/openhack/tui.json`.
...
Dodajte `openhack.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se OpenHack pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OPENHACK_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENHACK_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.openhack` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.openhack`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`cybersec.org.za/config.json`**](https://cybersec.org.za/config.json).

TUI konfiguracija koristi [**`cybersec.org.za/tui.json`**](https://cybersec.org.za/tui.json).
...
Koristite `OPENHACK_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `openhack.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `openhack serve` i `openhack web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš OpenHack server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `openhack.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj OpenHack konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, OpenHack pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/openhack/agents/` ili `.openhack/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, OpenHack će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`openhack run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/openhack/commands/` ili `.openhack/commands/`. [Saznajte više ovdje](/docs/commands).
...
OpenHack će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, OpenHack **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju OpenHack sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.openhack/plugins/` ili `~/.config/openhack/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite OpenHack da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
