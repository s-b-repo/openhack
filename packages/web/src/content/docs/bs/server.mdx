---
title: Server
description: Komunicirajte s openhack serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `openhack serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji openhack klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `openhack.local` |
...
Postavite `OPENHACK_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `openhack`, ili postavite `OPENHACK_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `openhack serve` i za `openhack web`.
...
Kada pokrenete `openhack`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite openhack server za programsku interakciju sa openhack-om.
:::

Ova arhitektura omogucava openhack podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `openhack serve` da startate standalone server. Ako je openhack TUI vec pokrenut, `openhack serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OpenHack [IDE](/docs/ide) pluginovi.
...
openhack server izlaže sljedece API-je.
