---
title: Plugin
description: Scrivi plugin per estendere OpenHack.
---

I plugin ti permettono di estendere OpenHack agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di OpenHack.
...
- `.openhack/plugins/` - plugin a livello progetto
- `~/.config/openhack/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/openhack/node_modules/`.
...
1. Config globale (`~/.config/openhack/openhack.json`)
2. Config di progetto (`openhack.json`)
3. Directory plugin globale (`~/.config/openhack/plugins/`)
4. Directory plugin di progetto (`.openhack/plugins/`)
...
OpenHack esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di openhack per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere openhack.
...
:::note
Se usi l'app desktop di OpenHack, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a openhack di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a openhack:
...
L'helper `tool` crea uno strumento personalizzato che openhack può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in openhack insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://cybersec.org.za/docs/sdk) per i dettagli.
