---
title: Server
description: Interagisci con il server openhack via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `openhack serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client openhack.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `openhack.local` |
...
Imposta `OPENHACK_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `openhack`, oppure imposta `OPENHACK_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `openhack serve` sia per `openhack web`.
...
Quando esegui `openhack` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server openhack per interagire con openhack in modo programmatico.
:::

Questa architettura permette a openhack di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `openhack serve` per avviare un server standalone. Se la TUI di openhack e gia in esecuzione, `openhack serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di OpenHack.
...
Il server openhack espone le seguenti API.
