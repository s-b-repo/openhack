...
Le skill degli agenti permettono a OpenHack di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OpenHack cerca in queste posizioni:

- Config di progetto: `.openhack/skills/<name>/SKILL.md`
- Config globale: `~/.config/openhack/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OpenHack risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.openhack/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/openhack/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.openhack/skills/git-release/SKILL.md` cosi':
...
OpenHack elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `openhack.json`:
...
**Per agenti integrati** (in `openhack.json`):
