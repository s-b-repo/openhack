---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OpenHack.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenHack fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OpenHack avvia la TUI per la directory corrente.
...
Quando usi la TUI di OpenHack, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OpenHack. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da OpenHack. _Alias_: `/quit`, `/q`
...
Questo è separato da `openhack.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo OpenHack attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `openhack.default`.
