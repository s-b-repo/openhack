...
Puoi aggiungere strumenti esterni a OpenHack usando il _Model Context Protocol_, o MCP. OpenHack supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [OpenHack Config](https://cybersec.org.za/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/openhack`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
OpenHack gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, OpenHack:
...
Se il server richiede autenticazione, OpenHack ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `openhack mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, OpenHack memorizzera' i token in modo sicuro in `~/.local/share/openhack/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in OpenHack insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione OpenHack come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare OpenHack al tuo account Sentry.
