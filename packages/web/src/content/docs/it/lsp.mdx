---
title: Server LSP
description: OpenHack si integra con i tuoi server LSP.
---

OpenHack può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

OpenHack include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `OPENHACK_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e openhack apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di openhack.
