...
OpenHack Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in OpenHack. Ti abboni a OpenHack Go e
...
OpenHack Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

OpenHack Go funziona come qualsiasi altro provider in OpenHack.

1. Accedi a **<a href={console}>OpenHack Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `OpenHack Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a OpenHack Go.
:::
...
OpenHack Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua OpenHack config
utilizza il formato `openhack-go/<model-id>`. Ad esempio, per Kimi K2.7 Code, useresti
`openhack-go/kimi-k2.7-code` nella tua configurazione.
...
Abbiamo creato OpenHack Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con OpenHack.
