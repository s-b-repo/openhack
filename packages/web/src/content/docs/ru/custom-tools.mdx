...
description: Создавайте инструменты, которые LLM может вызывать в openhack.
---

Пользовательские инструменты — это создаваемые вами функции, которые LLM может вызывать во время разговоров. Они работают вместе со [встроенными инструментами ](/docs/tools) openhack, такими как `read`, `write` и `bash`.
...
- Локально, поместив их в каталог `.openhack/tools/` вашего проекта.
- Или глобально, поместив их в `~/.config/openhack/tools/`.
