...
Создайте `.openhack/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию openhack или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем openhack [config](/docs/config):
...
- Глобальный: `~/.config/openhack/commands/`
- Для каждого проекта: `.openhack/commands/`
...
openhack включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
