...
Créez `.openhack/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration OpenHack ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre OpenHack [config](/docs/config) :
...
openhack comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
