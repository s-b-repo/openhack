---
title: SDK
description: Client JS de type sécurisé pour le serveur openhack.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK openhack JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler openhack par programme.
...
Créez une instance de openhack :

```javascript
import { createOpenhack } from "@openhack-ai/sdk"

const { client } = await createOpenhack()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `openhack.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${openhack.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de openhack, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@openhack-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*openhack" },
})
