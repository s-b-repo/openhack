---
title: Serveur
description: Interagissez avec le serveur openhack via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `openhack serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client openhack peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `openhack.local` |
...
Définissez `OPENHACK_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `openhack`, ou définissez `OPENHACK_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `openhack serve` et à `openhack web`.
...
Lorsque vous exécutez `openhack`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur openhack pour interagir avec openhack par programmation.
:::

Cette architecture permet à openhack de prendre en charge plusieurs clients et vous permet d'interagir avec openhack par programme.

Vous pouvez exécuter `openhack serve` pour démarrer un serveur autonome. Si vous avez le
openhack TUI en cours d'exécution, `openhack serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OpenHack [IDE](/docs/ide).
...
Le serveur openhack expose les API suivantes.
