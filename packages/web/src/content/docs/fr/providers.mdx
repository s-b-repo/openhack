---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en OpenHack.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHack utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration OpenHack.
...
en `~/.local/share/openhack/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre OpenHack
configuration.
...
OpenHack Zen est une liste de modèles fournis par l'équipe OpenHack qui ont été
testé et vérifié pour fonctionner correctement avec OpenHack. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par OpenHack Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez openhack et dirigez-vous vers [cybersec.org.za/auth](https://cybersec.org.za/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans OpenHack et son utilisation est totalement facultative.

---

## OpenHack Go

OpenHack Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe OpenHack qui ont été
testé et vérifié pour fonctionner correctement avec OpenHack.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `OpenHack Go` et rendez-vous sur [cybersec.org.za/auth](https://cybersec.org.za/zen).
...
Pour utiliser Amazon Bedrock avec OpenHack :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de openhack :
...
Pour une configuration spécifique au projet ou persistante, utilisez `openhack.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans OpenHack n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer openhack pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que openhack fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration openhack.
...
pour stocker le jeton dans le stockage d'authentification openhack.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
OpenHack utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller OpenHack
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `openhack.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `openhack.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec openhack :
...
Pour utiliser Google Vertex AI avec OpenHack :
...
Définissez-les lors de l'exécution de openhack.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via openhack, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [openhack-helicone-session](https://github.com/H2Shami/openhack-helicone-session) pour enregistrer automatiquement chaque conversation OpenHack en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation OpenHack répertoriée comme une session distincte.
...
Vous pouvez configurer openhack pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer openhack pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer openhack pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour OpenHack. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/openhack) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec OpenHack :
...
3. Copiez la clé API à utiliser dans OpenHack.
...
6. **Important** : Avant d'utiliser des modèles cloud dans OpenHack, vous devez extraire les informations du modèle localement :
...
OpenHack Zen est une liste de modèles testés et vérifiés fournie par l'équipe OpenHack. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>OpenHack Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **OpenHack Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé OpenHack API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration openhack.
...
5. Vous pouvez également les personnaliser via votre configuration openhack. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Openhack :
...
Vous pouvez également personnaliser les modèles via votre configuration openhack. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec openhack. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `openhack.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à OpenHack de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `openhack auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration openhack et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration openhack.
