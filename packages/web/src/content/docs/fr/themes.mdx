...
Avec OpenHack, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OpenHack utilise notre propre thème `openhack`.
...
OpenHack est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OpenHack corresponde à l'apparence de leur terminal
...
OpenHack prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/openhack/themes/*.json` ou `$XDG_CONFIG_HOME/openhack/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.openhack/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.openhack/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/openhack/themes
vim ~/.config/openhack/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .openhack/themes
vim .openhack/themes/my-theme.json
```
...
{
  "$schema": "https://cybersec.org.za/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
