---
title: Plugins
description: Écrivez vos propres plugins pour étendre OpenHack.
---

Les plugins vous permettent d'étendre OpenHack en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OpenHack.
...
- `.openhack/plugins/` - Plugins au niveau du projet
- `~/.config/openhack/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/openhack/node_modules/`.
...
1. Configuration globale (`~/.config/openhack/openhack.json`)
2. Configuration du projet (`openhack.json`)
3. Répertoire global des plugins (`~/.config/openhack/plugins/`)
4. Répertoire des plugins du projet (`.openhack/plugins/`)
...
OpenHack exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK openhack pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre openhack.
...
:::note
Si vous utilisez l'application de bureau OpenHack, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez openhack de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à openhack :
...
L'assistant `tool` crée un outil personnalisé que openhack peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour openhack aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://cybersec.org.za/docs/sdk) pour plus de détails.
