...
Les compétences d'agent permettent à OpenHack de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
OpenHack recherche ces emplacements :

- Configuration du projet : `.openhack/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/openhack/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, OpenHack remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.openhack/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/openhack/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.openhack/skills/git-release/SKILL.md` comme ceci :
...
OpenHack répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `openhack.json` :
...
**Pour les agents intégrés** (dans `openhack.json`) :
