...
OpenHack Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, OpenHack'daki diğer sağlayıcılar gibi çalışır. OpenHack Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve OpenHack'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OpenHack Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

OpenHack Go, OpenHack'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>OpenHack Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `OpenHack Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye OpenHack Go'ya abone olabilir.
:::
...
OpenHack Go aşağıdaki limitleri içerir:
...
OpenHack yapılandırmanızdaki [model id](/docs/config/#models) formatı `openhack-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `openhack-go/kimi-k2.7-code` kullanmalısınız.
...
OpenHack Go'yu şu amaçlarla oluşturduk:
...
4. OpenHack ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
