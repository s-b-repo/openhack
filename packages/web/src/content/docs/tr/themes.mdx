...
openhack ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak openhack kendi `openhack` temasını kullanır.
...
openhack birden fazla yerleşik temayla gelir.
...
- openhack'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
openhack, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/openhack/themes/*.json` veya `$XDG_CONFIG_HOME/openhack/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.openhack/themes/*.json`
4. **Geçerli çalışma dizini** - `./.openhack/themes/*.json`
