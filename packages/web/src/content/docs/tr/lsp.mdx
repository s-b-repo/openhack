...
description: OpenHack, LSP sunucularınızla bütünleşir.
---

OpenHack, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

OpenHack, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`OPENHACK_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve openhack bir dosyayı açtığında:
...
LSP sunucularını, openhack yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
