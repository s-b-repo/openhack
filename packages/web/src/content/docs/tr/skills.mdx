...
Ajan becerileri, openhack'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
openhack su konumlari tarar:

- Proje konfigurasyonu: `.openhack/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/openhack/skills/<name>/SKILL.md`
...
Proje yerel yollarinda openhack, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.openhack/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/openhack/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.openhack/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
openhack, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`openhack.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`openhack.json` icinde):
