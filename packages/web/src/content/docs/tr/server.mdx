---
title: Sunucu
description: openhack sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`openhack serve` komutu, openhack istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `openhack.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OPENHACK_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `openhack` değeridir; değiştirmek isterseniz `OPENHACK_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `openhack serve` hem de `openhack web` için geçerlidir.
...
`openhack` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
openhack ile programatik etkileşim için openhack sunucusunu kullanın.
:::

Bu mimari, openhack'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`openhack serve` ile bağımsız bir sunucu başlatabilirsiniz. openhack TUI açıksa, `openhack serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum openhack [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
openhack sunucusu aşağıdaki API'leri sunar.
