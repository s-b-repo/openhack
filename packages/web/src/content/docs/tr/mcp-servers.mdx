...
_Model Bağlam Protokolü_ veya MCP kullanarak openhack'a harici araçlar ekleyebilirsiniz. openhack hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [openhack Config](https://cybersec.org.za/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/openhack` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
openhack, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde openhack şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa openhack, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `openhack mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından openhack, belirteçleri `~/.local/share/openhack/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra openhack'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi openhack yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve openhack'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
