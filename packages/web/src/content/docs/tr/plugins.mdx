---
title: Eklentiler
description: OpenHack'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek OpenHack'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya OpenHack'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.openhack/plugins/` - Proje düzeyinde eklentiler
- `~/.config/openhack/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/openhack/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/openhack/openhack.json`)
2. Project config (`openhack.json`)
3. Global eklenti dizini (`~/.config/openhack/plugins/`)
4. Proje eklenti dizini (`.openhack/plugins/`)
...
OpenHack bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir OpenHack SDK istemcisi.
...
OpenHack'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
OpenHack masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
OpenHack'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca OpenHack'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, OpenHack'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra OpenHack'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://cybersec.org.za/docs/sdk) bakın.
