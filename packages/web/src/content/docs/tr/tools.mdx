...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. openhack yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
openhack içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `OPENHACK_EXPERIMENTAL_LSP_TOOL=true` (veya `OPENHACK_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca openhack provider kullanılırken veya `OPENHACK_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

openhack başlatırken etkinleştirmek için:
