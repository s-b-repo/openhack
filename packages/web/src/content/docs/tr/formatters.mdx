...
description: openhack dile özgü biçimlendiriciler kullanır.
---

openhack, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

openhack, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, openhack bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

openhack bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri openhack yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
