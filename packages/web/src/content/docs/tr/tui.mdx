---
title: TUI
description: OpenHack terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenHack, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

OpenHack'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
OpenHack TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
OpenHack'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
OpenHack'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `openhack.json` dosyasından ayrıdır.
...
- `leader_timeout` - OpenHack'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
...
Özel bir TUI yapılandırma yolu yüklemek için `OPENHACK_TUI_CONFIG` kullanın.

### Attention

Attention, OpenHack bir yanıt beklediğinde, izin onayı gerektiğinde, oturum hatası oluştuğunda veya oturum tamamlandığında TUI'nin sizi ses ve masaüstü bildirimiyle uyarmasını sağlar. `attention.enabled` ile etkinleştirin; yerleşik olaylar gerçekleştiğinde ses çalar. Masaüstü bildirimleri yalnızca terminal penceresi odakta değilken gönderilir ve subagent olayları için kullanılmaz.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `openhack.default`.
