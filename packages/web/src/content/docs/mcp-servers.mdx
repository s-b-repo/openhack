...
You can add external tools to OpenHack using the _Model Context Protocol_, or MCP. OpenHack supports both local and remote servers.
...
You can define MCP servers in your [OpenHack Config](https://cybersec.org.za/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/openhack` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OpenHack automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenHack will:
...
If the server requires authentication, OpenHack will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `openhack mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenHack will store the tokens securely in `~/.local/share/openhack/mcp-auth.json`.
...
Your MCPs are available as tools in OpenHack, alongside built-in tools. So you can manage them through the OpenHack config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenHack to your Sentry account.
