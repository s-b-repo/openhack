...
مع OpenHack يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OpenHack سمتنا `openhack`.
...
يأتي OpenHack مع عدة سمات مدمجة.
...
- يريدون أن يطابق OpenHack مظهر terminal لديهم
...
يدعم OpenHack نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/openhack/themes/*.json` أو `$XDG_CONFIG_HOME/openhack/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.openhack/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.openhack/themes/*.json`
