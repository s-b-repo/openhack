---
title: الخادم
description: تفاعل مع خادم openhack عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `openhack serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل openhack استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `openhack.local` |
...
عيّن `OPENHACK_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `openhack`، أو عيّن `OPENHACK_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `openhack serve` و `openhack web`.
...
عند تشغيل `openhack` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم openhack للتفاعل مع openhack برمجيا.
:::

تتيح هذه البنية لـ openhack دعم عدة عملاء وتمكّنك من التفاعل مع openhack برمجيا.

يمكنك تشغيل `openhack serve` لبدء خادم مستقل. إذا كانت واجهة openhack في terminal (TUI)
قيد التشغيل، فسيبدأ `openhack serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات OpenHack لـ [IDE](/docs/ide).
...
يعرض خادم openhack واجهات API التالية.
