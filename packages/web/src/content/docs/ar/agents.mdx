...
يوجد نوعان من الوكلاء في OpenHack: وكلاء أساسيون ووكلاء فرعيون.
...
يأتي OpenHack مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي OpenHack مع ثلاثة وكلاء فرعيين مدمجين: **General** و **Explore** و **Scout**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي OpenHack مع وكيلين أساسيين مدمجين وثلاثة وكلاء فرعيين مدمجين.
...
وكيل للقراءة فقط مخصص للوثائق الخارجية وأبحاث التبعيات. استخدمه عندما تحتاج إلى استنساخ مستودع تبعية داخل ذاكرة التخزين المؤقت المُدارة في OpenHack، أو فحص الشفرة المصدرية لمكتبة، أو إجراء مراجع متقاطعة بين الشفرة المحلية والتنفيذات upstream بدون تعديل مساحة العمل الخاصة بك.
...
هيّئ الوكلاء في ملف إعدادات `openhack.json`:
...
- عام: `~/.config/openhack/agents/`
- لكل مشروع: `.openhack/agents/`
...
إذا لم يتم تحديد temperature، يستخدم OpenHack القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات OpenHack العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات OpenHack الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [OpenHack Zen](/docs/zen)، فستستخدم `openhack/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `openhack models` لعرض قائمة بالنماذج المتاحة.
:::
...
:::tip
هل لديك وكيل تود مشاركته؟ [قدّم PR](https://github.com/anomalyco/openhack).
:::
