---
title: المزوّدون
description: استخدام أي موفّر LLM في OpenHack.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OpenHack ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OpenHack.
...
في `~/.local/share/openhack/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OpenHack.
...
OpenHack Zen هي قائمة نماذج يوفّرها فريق OpenHack وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenHack. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenHack Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر openhack، ثم انتقل إلى [cybersec.org.za/auth](https://cybersec.org.za/auth).
...
يعمل مثل أي مزوّد آخر في OpenHack واستخدامه اختياري بالكامل.

---

## OpenHack Go

OpenHack Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق OpenHack، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenHack.

1. شغّل الأمر `/connect` في TUI، واختر `OpenHack Go`، ثم انتقل إلى [cybersec.org.za/auth](https://cybersec.org.za/zen).
...
لاستخدام Amazon Bedrock مع OpenHack:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل openhack:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `openhack.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OpenHack غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين openhack لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل openhack بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات openhack.
...
تخزين الرمز في مساحة تخزين مصادقة openhack.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم OpenHack نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OpenHack
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`openhack.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع openhack:
...
لاستخدام Google Vertex AI مع OpenHack:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر openhack، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [openhack-helicone-session](https://github.com/H2Shami/openhack-helicone-session) لتسجيل كل محادثة OpenHack تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OpenHack مدرجة كجلسة مستقلة.
...
يمكنك ضبط openhack لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط openhack لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط openhack لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenHack. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/openhack) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OpenHack:
...
3. انسخ مفتاح API لاستخدامه في OpenHack.
...
6. **مهم**: قبل استخدام نماذج السحابة في OpenHack، يجب سحب معلومات النموذج محليا:
...
OpenHack Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OpenHack. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OpenHack Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OpenHack Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OpenHack.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات openhack.
...
5. يمكنك أيضا تخصيصها عبر إعدادات openhack. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Openhack:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات openhack. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع openhack. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `openhack.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ OpenHack بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `openhack auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات openhack وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات openhack.
