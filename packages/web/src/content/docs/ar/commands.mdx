...
أنشئ الملف `.openhack/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OpenHack أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OpenHack:
...
- على مستوى النظام: `~/.config/openhack/commands/`
- لكل مشروع: `.openhack/commands/`
...
يتضمن openhack عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
