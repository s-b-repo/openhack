...
description: Cree herramientas que LLM pueda llamar en openhack.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de openhack como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.openhack/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/openhack/tools/`.
...
```ts title=".openhack/tools/database.ts" {1}
...
```ts title=".openhack/tools/math.ts"
import { tool } from "@openhack-ai/plugin"
...
```ts title=".openhack/tools/bash.ts"
import { tool } from "@openhack-ai/plugin"
...
```ts title=".openhack/tools/project.ts" {8}
import { tool } from "@openhack-ai/plugin"
...
```python title=".openhack/tools/add.py"
import sys
...
```ts title=".openhack/tools/python-add.ts" {10}
import { tool } from "@openhack-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".openhack/tools/add.py")
