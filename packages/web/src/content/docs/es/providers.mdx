---
title: Proveedores
description: Usando cualquier proveedor LLM en OpenHack.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHack usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración OpenHack.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu OpenHack
configuración.
...
OpenHack Zen es una lista de modelos proporcionados por el equipo OpenHack que han sido
probado y verificado para funcionar bien con OpenHack. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con OpenHack Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione openhack y diríjase a [cybersec.org.za/auth](https://cybersec.org.za/auth).
...
Funciona como cualquier otro proveedor en OpenHack y su uso es completamente opcional.

---

## OpenHack Go

OpenHack Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de OpenHack que han sido
probado y verificado para funcionar bien con OpenHack.

1. Ejecute el comando `/connect` en TUI, seleccione `OpenHack Go` y diríjase a [cybersec.org.za/auth](https://cybersec.org.za/zen).
...
Para usar Amazon Bedrock con OpenHack:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta openhack:
   ...
   Para una configuración persistente o específica del proyecto, utilice `openhack.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en OpenHack no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar openhack para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que openhack funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración openhack.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `OpenHack`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación openhack.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
OpenHack utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear OpenHack
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `openhack.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `openhack.json`:
...
Para utilizar su suscripción GitHub Copilot con openhack:
...
Para utilizar Google Vertex AI con OpenHack:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta openhack.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de openhack, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [openhack-helicone-session](https://github.com/H2Shami/openhack-helicone-session) para registrar automáticamente cada conversación OpenHack como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación OpenHack enumerada como una sesión separada.
...
Puede configurar openhack para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar openhack para usar modelos locales a través de LM Studio.
...
Puedes configurar openhack para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para OpenHack. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/openhack) para obtener más detalles.
:::
...
Para usar Ollama Cloud con OpenHack:
...
3. Copie la clave API para usarla en OpenHack.
...
6. **Importante**: Antes de usar modelos en la nube en OpenHack, debe obtener la información del modelo localmente:
...
OpenHack Zen es una lista de modelos probados y verificados proporcionada por el equipo OpenHack. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>OpenHack Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **OpenHack Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave OpenHack API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración openhack.
...
5. También puedes personalizarlos a través de tu configuración openhack. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con OpenHack:
...
También puede personalizar modelos a través de su configuración openhack. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con openhack. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `openhack.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a OpenHack comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `openhack auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración openhack y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración openhack.
