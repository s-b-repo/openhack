...
Las herramientas permiten que LLM realice acciones en su código base. OpenHack viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en OpenHack.
...
:::note
Esta herramienta solo está disponible cuando `OPENHACK_EXPERIMENTAL_LSP_TOOL=true` (o `OPENHACK_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenHack o cuando la variable de entorno `OPENHACK_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenHack:

```bash
OPENHACK_ENABLE_EXA=1 openhack
```
