...
Las habilidades del agente permiten a OpenHack descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
OpenHack busca estas ubicaciones:

- Configuración del proyecto: `.openhack/skills/<name>/SKILL.md`
- Configuración global: `~/.config/openhack/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, OpenHack sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.openhack/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/openhack/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.openhack/skills/git-release/SKILL.md` así:
...
OpenHack enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `openhack.json`:
...
**Para agentes integrados** (en `openhack.json`):
