---
title: Servidor
description: Interactuar con el servidor openhack a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `openhack serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente openhack puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `openhack.local` |
...
Configure `OPENHACK_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `openhack`, o configure `OPENHACK_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `openhack serve` como a `openhack web`.
...
Cuando ejecuta `openhack`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor openhack para interactuar con openhack mediante programación.
:::

Esta arquitectura permite que openhack admita múltiples clientes y le permite interactuar con openhack mediante programación.

Puede ejecutar `openhack serve` para iniciar un servidor independiente. Si tienes el
openhack TUI ejecutándose, `openhack serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OpenHack [IDE](/docs/ide).
...
El servidor openhack expone las siguientes API.
