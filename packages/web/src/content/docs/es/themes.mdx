...
Con OpenHack puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, OpenHack usa nuestro propio tema `openhack`.
...
OpenHack viene con varios temas integrados.
...
- Quiere que OpenHack coincida con la apariencia de su terminal
...
OpenHack admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/openhack/themes/*.json` o `$XDG_CONFIG_HOME/openhack/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.openhack/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.openhack/themes/*.json`
