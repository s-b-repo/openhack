---
title: Complementos
description: Escriba sus propios complementos para extender OpenHack.
---

Los complementos le permiten extender OpenHack al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OpenHack.
...
- `.openhack/plugins/` - Complementos a nivel de proyecto
- `~/.config/openhack/plugins/` - Complementos globales
...
```json title="openhack.json"
{
  ...
  "plugin": ["openhack-helicone-session", "openhack-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/openhack/node_modules/`.
...
1. Configuración global (`~/.config/openhack/openhack.json`)
2. Configuración del proyecto (`openhack.json`)
3. Directorio global de complementos (`~/.config/openhack/plugins/`)
4. Directorio de complementos del proyecto (`.openhack/plugins/`)
...
```json title=".openhack/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
OpenHack ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".openhack/plugins/my-plugin.ts"
...
```js title=".openhack/plugins/example.js"
...
- `client`: Un cliente SDK openhack para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar openhack.
...
```js title=".openhack/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "openhack"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio OpenHack, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que openhack lea archivos `.env`:

```javascript title=".openhack/plugins/env-protection.js"
...
```javascript title=".openhack/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a openhack:

```ts title=".openhack/plugins/custom-tools.ts"
import { type Plugin, tool } from "@openhack-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que openhack puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para openhack junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://cybersec.org.za/docs/sdk) para obtener más detalles.
...
```ts title=".openhack/plugins/compaction.ts"
import type { Plugin } from "@openhack-ai/plugin"
...
```ts title=".openhack/plugins/custom-compaction.ts"
import type { Plugin } from "@openhack-ai/plugin"
