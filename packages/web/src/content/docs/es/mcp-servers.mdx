...
Puede agregar herramientas externas a OpenHack utilizando el _Model Context Protocol_ o MCP. OpenHack admite servidores locales y remotos.
...
Puede definir servidores MCP en su [OpenHack Config](https://cybersec.org.za/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/openhack`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
OpenHack maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, OpenHack:
...
Si el servidor requiere autenticación, OpenHack le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `openhack mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, OpenHack almacenará los tokens de forma segura en `~/.local/share/openhack/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en OpenHack, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración OpenHack como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar OpenHack a su cuenta Sentry.
