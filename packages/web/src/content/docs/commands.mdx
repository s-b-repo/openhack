...
Create `.openhack/commands/test.md`:
...
You can add custom commands through the OpenHack config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OpenHack [config](/docs/config):
...
openhack includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
