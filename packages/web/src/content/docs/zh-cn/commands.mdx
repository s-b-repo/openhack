...
你可以通过 OpenHack 配置或在 `commands/` 目录中创建 markdown 文件来添加自定义命令。

---

### JSON

在 OpenHack [配置](/docs/config)中使用 `command` 选项：
...
- 全局：`~/.config/openhack/commands/`
- 项目级：`.openhack/commands/`
...
openhack 包含多个内置命令，如 `/init`、`/undo`、`/redo`、`/share`、`/help`；[了解更多](/docs/tui#commands)。
