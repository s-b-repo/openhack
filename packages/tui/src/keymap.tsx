...
export const OPENHACK_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OPENHACK_MODE_KEY = "openhack.mode"

export const OpenhackKeymapProvider = KeymapProvider
export const useOpenhackKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type OpenhackModeStack = ReturnType<typeof createOpenhackModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, OpenhackModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createOpenhackModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(OPENHACK_MODE_KEY, OPENHACK_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OPENHACK_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OPENHACK_MODE_KEY, stack.at(-1)?.mode ?? OPENHACK_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OPENHACK_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OPENHACK_MODE_KEY, undefined)
...
export function useOpenhackModeStack() {
  return getOpenhackModeStack(useOpenhackKeymap())
}

export function getOpenhackModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Openhack mode stack is not registered for this keymap")
  return value
}
...
export function registerOpenhackKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createOpenhackModeStack(keymap)
