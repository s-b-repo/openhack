import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@openhack-ai/plugin/tui"
...
import type { useOpenhackKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useOpenhackKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getOpenhackModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getOpenhackModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
