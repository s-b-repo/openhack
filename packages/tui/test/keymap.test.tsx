...
import { getOpenhackModeStack, OPENHACK_BASE_MODE, OpenhackKeymapProvider, registerOpenhackKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerOpenhackKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when openhack mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getOpenhackModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getOpenhackModeStack(keymap).push("autocomplete")
