...
import { createOpenhack, type ToolPart } from "@openhack-ai/sdk"
...
console.log("🚀 Starting openhack server...")
const openhack = await createOpenhack({
  port: 0,
})
console.log("✅ Openhack server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await openhack.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new openhack session...")
    const { client, server } = openhack
    ...
    console.log("✅ Created openhack session:", createResult.data.id)
  ...
  console.log("📝 Sending to openhack:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Openhack response:", JSON.stringify(result, null, 2))
