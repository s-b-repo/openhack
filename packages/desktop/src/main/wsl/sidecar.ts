...
import { type WslCommandLine, resolveWslOpenhack, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const openhack = await resolveWslOpenhack(distro)
  if (!openhack) throw new Error(`OpenHack is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "openhack"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export OPENHACK_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export OPENHACK_CLIENT=desktop",
    `export OPENHACK_SERVER_USERNAME=${shellEscape(username)}`,
    `export OPENHACK_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(openhack)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
