...
let releaseOpenhackResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectOpenhackVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectOpenhackVersion("1.14.35", "1.16.2")).toThrow(
    "OpenHack update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating OpenHack", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.openhack/bin/openhack",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, openhackChecks: {} })
})
...
test("ignores stale background OpenHack checks after removing a WSL server", async () => {
  persistedServers = []
  releaseOpenhackResolve = undefined
  ...
  await waitFor(() => !!releaseOpenhackResolve)
  await controller.removeServer("wsl:Debian")
  releaseOpenhackResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().openhackChecks).toEqual({})
})

test("ignores stale startup OpenHack checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking OpenHack", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const openhack: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOpenhack: async (distro) => {
      openhack.push(distro)
      return "/home/me/.openhack/bin/openhack"
    },
  })
  ...
  expect(openhack).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(openhack).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().openhackChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check OpenHack in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const openhack: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(openhack).toEqual(["Debian"])
  expect(Object.keys(controller.getState().openhackChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveOpenhack: async () => {
      await new Promise<void>((resolve) => {
        releaseOpenhackResolve = resolve
      })
      return "/home/me/.openhack/bin/openhack"
    },
  }
