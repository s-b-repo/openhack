...
export async function installWslOpenhack(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
      ["bash", "-lc", `curl -fsSL https://cybersec.org.za/install | bash -s -- --version ${shellEscape(version)}`],
      distro,
    ),
    withTimeout(opts, DEFAULT_WSL_INSTALL_TIMEOUT_MS),
    DEFAULT_WSL_INSTALL_TIMEOUT_MS,
...
export async function resolveWslOpenhack(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.openhack/bin/openhack" ]; then printf "%s\\n" "$HOME/.openhack/bin/openhack"; fi',
        distro,
        opts,
      )
    ).stdout,
