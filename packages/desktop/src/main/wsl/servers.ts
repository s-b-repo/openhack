...
import { expectOpenhackVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveOpenhack?: typeof resolveWslOpenhack
...
export function createWslServersController(
  ...
  const setOpenhackCheck = (distro: string, check: WslOpenhackCheck) => {
    setState({
      openhackChecks: {
        ...state.openhackChecks,
        [distro]: check,
      },
    })
  }

  const checkOpenhack = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveOpenhack ?? resolveWslOpenhack)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return openhackCheck(distro, resolved, version, appVersion)
  }

  const refreshOpenhackCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setOpenhackCheck(distro, await checkOpenhack(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const openhackChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.openhackChecks[distro])
        .map(async (distro) => [distro, await checkOpenhack(distro, opts)] as const),
    )
    if (openhackChecks.length) {
      setState({ openhackChecks: { ...state.openhackChecks, ...Object.fromEntries(openhackChecks) } })
    }
  }
  ...
  const refreshOpenhackCheckBackground = (id: string, distro: string) => {
    void checkOpenhack(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setOpenhackCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl openhack check failed", { id, distro, message })
      })
  }

  const refreshOpenhackChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkOpenhack(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setOpenhackCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl openhack check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshOpenhackCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installOpenhack(name: string) {
      await runJob({ kind: "install-openhack", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslOpenhack(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "OpenHack installation failed")
        }
        await refreshOpenhackCheck(name, { signal: abort.signal })
        expectOpenhackVersion(state.openhackChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.openhackChecks, distro) : {}),
      })
    },
...
function openhackCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "openhack is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "openhack is installed but could not run",
    }
  }
  return {
    distro,
