...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.openhack.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `openhack.settings.dat` is special: it maps to the `openhack.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"openhack.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // openhack.settings.dat → the electron settings store ("openhack.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "openhack.global.dat").
  const storeName = filename === "openhack.settings.dat" ? "openhack.settings" : filename
