...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve openhack route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "openhack")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "openhack")).toBe("openai")
    expect(statProvider("big-pickle", "", "openhack")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "openhack"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep openhack as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "openhack"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "openhack"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep openhack or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "openhack"), country: "US" })).toMatchObject([
