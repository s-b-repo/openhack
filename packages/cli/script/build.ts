...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      OPENHACK_CLI_NAME: `'${binary}'`,
      OPENHACK_MODELS_DEV: modelsData,
      OPENHACK_CHANNEL: `'${Script.channel}'`,
      OPENHACK_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@openhack-ai/${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/openhack.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
