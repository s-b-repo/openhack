...
import { createOpenhackClient } from "@openhack-ai/sdk/v2/client"
import { ServerAuth } from "@openhack-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createOpenhackClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@openhack/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createOpenhackClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createOpenhackClient({ baseUrl: connection.url, headers: connection.headers })
    })
