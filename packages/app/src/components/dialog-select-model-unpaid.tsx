            ...
            <Tooltip
              ...
              value={
                <ModelTooltip
                  model={item}
                  latest={item.latest}
                  free={item.provider.id === "openhack" && (!item.cost || item.cost.input === 0)}
                />
              }
            >
              {node}
              ...
              >
                {(i) => (
                  <div class="w-full flex items-center gap-x-3">
                    ...
                    <span>{i.name}</span>
                    <Show when={i.id === "openhack"}>
                      <div class="text-14-regular text-text-weak">{language.t("dialog.provider.openhack.tagline")}</div>
                    </Show>
                    <Show when={i.id === "openhack"}>
                      <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                    </Show>
                    <Show when={i.id === "openhack-go"}>
                      <>
                        <div class="text-14-regular text-text-weak">
                          {language.t("dialog.provider.openhackGo.tagline")}
                        </div>
                        <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                      </>
                    </Show>
