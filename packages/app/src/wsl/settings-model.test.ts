...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when OpenHack needs attention", () => {
    expect(wslOpenhackAction(undefined)).toBeUndefined()
    expect(
      wslOpenhackAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install OpenHack")
    expect(
      wslOpenhackAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/openhack",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update OpenHack")
    expect(
      wslOpenhackAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/openhack",
  ...
  test("does not report ready when OpenHack is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        openhackChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.openhack/bin/openhack",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installOpenhack" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-openhack")
  })
