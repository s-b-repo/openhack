...
import { Tag } from "@openhack-ai/ui/v2/badge-v2"
...
import { wslOpenhackAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.openhackChecks[item.config.distro]
          const openhackAction = () => wslOpenhackAction(check())
          const busy = () => wsl.data?.job?.kind === "install-openhack" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={openhackAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installOpenhack(item.config.distro))}
                    >
