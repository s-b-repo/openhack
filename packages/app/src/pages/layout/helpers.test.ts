...
import { type Session } from "@openhack-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("openhack://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("openhack://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("openhack://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("openhack://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("openhack://open-project")).toBeUndefined()
    expect(parseDeepLink("openhack://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "openhack://open-project?directory=/a",
      "openhack://other?directory=/b",
      "openhack://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("openhack://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("openhack://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("openhack://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("openhack://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "openhack://new-session?directory=/a",
      "openhack://open-project?directory=/b",
      "openhack://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENHACK__: {
        deepLinks: ["openhack://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENHACK__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["openhack://open-project?directory=/a"])
