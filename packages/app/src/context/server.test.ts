  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://openhack.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://openhack.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://openhack.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://openhack.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://openhack.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://openhack.example.com"),
      ),
    ).toEqual({
