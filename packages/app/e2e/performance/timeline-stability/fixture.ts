...
import { SessionStatusEvent } from "@openhack-ai/schema/session-status-event"
import { SessionV1 } from "@openhack-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockOpenHackServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/OpenHack/TimelineStability"
...
export const model = { providerID: "openhack", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("openhack.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["openhack"],
    default: { providerID: "openhack", modelID: "claude-opus-4-6" },
  }
}
