...
| `src/frontier.ts` | `Frontier.{pruneWithEnforcement,pruneAndAnnotateEdges}` | Pure-enforcement pruning. Reuses `evaluateToolCall`, `ROE.enforce`, `ResourceManager.findConflicting` from `@opencode-ai/openhack`. |
...
- **Loop driver** — `packages/opencode/src/cli/cmd/openhack.automode.ts`. Extends `runOrchestrationLoop` behind the `graph.controller_enabled` config flag (default off). See the header in that file for the exact insertion points.
