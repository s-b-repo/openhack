...
# パッケージマネージャー
npm i -g openhack-ai@latest        # bun/pnpm/yarn でもOK
scoop install openhack             # Windows
choco install openhack             # Windows
brew install anomalyco/tap/openhack # macOS と Linux（推奨。常に最新）
brew install openhack              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S openhack            # Arch Linux (Stable)
paru -S openhack-bin               # Arch Linux (Latest from AUR)
mise use -g openhack               # どのOSでも
nix run nixpkgs#openhack           # または github:anomalyco/openhack で最新 dev ブランチ
```
...
OpenHack はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/openhack/releases) から直接ダウンロードするか、[cybersec.org.za/download](https://cybersec.org.za/download) を利用してください。
...
1. `$OPENHACK_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.openhack/bin` - デフォルトのフォールバック
...
OpenHack には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://cybersec.org.za/docs/agents) の詳細はこちら。

### ドキュメント

OpenHack の設定については [**ドキュメント**](https://cybersec.org.za/docs) を参照してください。

### コントリビュート

OpenHack に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### OpenHack の上に構築する

OpenHack に関連するプロジェクトで、名前に "openhack"（例: "openhack-dashboard" や "openhack-mobile"）を含める場合は、そのプロジェクトが OpenHack チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/openhack) | [X.com](https://x.com/openhack)
